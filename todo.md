...
> 文档状态：Active
> 当前基线：合并 PR #393 后的 `main`
> 基线提交：`3da0dd23aac0d4d4c3f40fa5fee762b100e8e069`
> 当前主里程碑：M2 — Observer-only 长程轨迹投影契约
> 最后更新：2026-07-23
...
| `M0-011B` child 成功后 Session revision 未提交的 crash reconciliation | `I/T/—` | `DONE` | 第二轮 PR-AS 与 PR-ATb 定点故障测试通过 |
...
优先级：`P0`
范围状态：`DONE`。本地两轮 runtime、第二轮 gate restart、post-child reconciliation 与 PR-ATb registration adoption 均已有对应证据。
...
| `M1-012` 第二轮 PR-AS 成功但 Session revision 未提交时 reconciliation | `I/T/—` | `DONE` | 定点 fault injection 后 inspect 接管，未重复 resume |
| `M1-013` PR-ATb 成功注册后重启且不重复调用 adapter | `I/T/—` | `DONE` | 定点 fault injection 后采用既有 publication，executor 调用为 0 |
...
| 任务 | 证据 | 工作状态 |
|---|---|---|
| `M2-001` 定义轨迹审计核心研究问题 | `—/—/—` | `READY` |
| `M2-002` 定义投影、审计和执行控制边界 | `—/—/—` | `READY` |
| `M2-003` v1 只能 post-hoc 观察和验证 | `—/—/—` | `READY` |
| `M2-004` 不保存或依赖 private chain-of-thought | `—/—/—` | `READY` |
| `M2-005` 仅保存 source-backed evidence、reason code、justification、expected/actual outcome | `—/—/—` | `READY` |
| `M2-006` alternatives 仅在源系统真实持久化时引用 | `—/—/—` | `READY` |
...
### 2026-07-23：PR-BB 第二轮定点恢复验收完成

- 决策：接受第二轮 PR-AS post-publication/pre-revision 与 PR-ATb post-registration/pre-revision 两处确定性故障注入测试。
- 结果：gated PR-AS 由 inspect reconcile，resume 不重复；PR-ATb 由下一次 advance 采用既有 execution record，恢复 executor 调用为 0。
- 边界：仅增加测试和路线证据，不改变 Session、Executor、科学 adapter 或 publication contract。
...
### 唯一当前动作：PR-BD observer-only trajectory projection v1

范围：从既有 terminal Session 事实做 post-hoc 投影；不得修改 Executor、PR-AU、PR-AV 或科学 adapter 主动写事件。

必须验证：

1. v1 只接受 terminal Session，并按 immutable revision 因果顺序投影。
2. typed authority、canonical ordering/serialization 和无绝对路径 identity 契约冻结。
3. 单轮与多轮 Session 都能生成确定性的 immutable projection publication。
4. mutable `action.json` 只能形成 telemetry 或 inconsistency finding，不得覆盖权威事实。
5. audit on/off 时原始 scientific artifact bytes 完全相同。
