...
| `M0-005B` PR-AU `request_generation_approval → Round 2` | `I/T/—` | `READY` | 等待 M1 runtime 验收 |
| `M0-006A` PR-AV durable single-round bounded session | `I/T/V` | `DONE` | 已验收 |
| `M0-006B` PR-AV durable multi-round bounded session | `I/T/—` | `READY` | 等待 M1 runtime 验收 |
...
| 任务 | 证据 | 工作状态 | 尚需完成 |
|---|---|---|---|
| `M1-001` 第一轮产生真实 property-qualified supply shortfall | `I/T/—` | `READY` | 运行代表性 exact-input canary |
| `M1-002` 第一轮 PR-AU 发布精确 generation authorization | `I/T/—` | `READY` | 登记 authorization |
| `M1-003` 第二轮 gate snapshot 绑定 controller、state fingerprint、count、authorization 和 gate | `I/T/—` | `READY` | runtime 核验字段 |
| `M1-004` 第二轮 PR-AS 不得退化为 direct/root PR-AS | `I/T/—` | `READY` | 核验 controller authorization |
| `M1-005` 第二轮 PR-ATb 使用完整有序 generation roster | `I/T/—` | `READY` | 执行 v2 receipt |
| `M1-006` PR-ATb 从全部原始 PR-AS publications 重建候选池 | `I/T/—` | `READY` | exact replay runtime evidence |
| `M1-007` 跨轮 SMILES、Standard InChI、InChIKey 去重 | `I/T/—` | `READY` | 正常和冲突证据 |
| `M1-008` Registry 与两轮 generated candidates 重新执行全局 constraints、percentiles、Pareto 和 rank | `I/T/—` | `READY` | 记录完整 pool 与 Top-N |
| `M1-009` PR-AU 累计预算与 generation roster 一致 | `I/T/—` | `READY` | 记录精确 usage 与停止原因 |
| `M1-010` 发布本地两轮 immutable runtime evidence | `—/—/—` | `READY` | 当前唯一动作 |
...
| 任务 | 证据 | 工作状态 | 尚需完成 |
|---|---|---|---|
| `M1-011` 第二轮 generation gate 在 `WAITING_USER` 重启并恢复 | `I/T(partial)/—` | `DEFERRED` | PR-BA 后执行第二轮定点 restart |
...
| `M1-001`～`M1-010` | — | — | — | runtime validation pending |
...
### 唯一当前动作：PR-BA 本地两轮 Session canary

范围：只运行和记录已有能力；除非 runtime 暴露 correctness blocker，否则不新增 schema 或科学策略。

必须验证：

1. Round 1 PR-ARb v2 为 incomplete，且不发布部分最终 Top-N。
2. PR-AU 发布精确的第二轮 generation authorization。
3. 在第二轮 generation gate 重启 PR-AW 服务并重新 inspect。
4. 第二轮 PR-AS exact 消费 predecessor authorization。
5. PR-ATb v2 cumulative roster 包含两份有序 PR-AS publication。
6. 全部 Registry 与 generated candidates 重新预测和全局排序。
7. 累计 usage 与第 2.3 节冻结值完全一致，以 `target_top_n_complete` 停止。
8. 新进程 terminal exact replay。
9. evidence 记录 exact commit、input SHA、child manifests、claims 和所有失败/中止尝试。

### 主线队列

```text
PR-BA  本地两轮 runtime canary
