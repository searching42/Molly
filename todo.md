...
> 文档状态：Active
> 当前基线：合并 PR #394 后的 `main`
> 基线提交：`e65381ce2a300834499e285ac2ad92cf32121d10`
...
- `[—/—/—] [READY] M2-022`：verifier 消费原始 Session、action records 和 child publications，重建 projection 并逐字节比较。
- `[—/—/—] [READY] M2-023`：完整重签名、事件删除/重排、source replacement 和 causal-link replacement fail closed。
...
- `[—/—/—] [READY] M2-GATE-006`：外部锚定 verifier 拒绝完整重签名攻击。
...
| `M1-012`～`M1-013` | #394 | PR-AV tests: `bbe68d92c4fedcecd13d62c984ea10acab0bc848`; PR-AW recovery: `0180f56396662f4b7f1f8e68ebf06bed003129df`; merge: pending | `tests/test_oled_bounded_discovery_session.py`, `tests/test_oled_bounded_discovery_session_api.py` | second-round Session and control-plane reconciliation tested |
...
### 唯一当前动作：PR-BE external-anchor trajectory verifier

范围：从原始 Session、immutable action request、StageState、gate decision、Artifact Registry 和 child publications 独立重建 PR-BD projection，并与 persisted publication 逐字节比较。

必须验证：

1. verifier 不能只验证 projection 内部自洽，必须重新消费外部权威源。
2. 重建的 `trajectory.json`、`events.jsonl`、`source_bindings.json` 和 `telemetry_findings.jsonl` 必须逐字节一致。
3. 完整重签名、事件删除/重排、source replacement、causal-link replacement 和 output roster 变化全部 fail closed。
4. mutable telemetry 冲突继续只形成 finding；immutable request 或科学 authority 变化必须拒绝。
5. verifier 只读且不得注册新的科学 trust anchor。

### 主线队列

```text
PR-BD  observer-only trajectory projection v1（本 PR 完成）
PR-BE  external-anchor trajectory verifier 与对抗测试（下一项）
```
