...
范围状态：`READY`。M3 已完成并提供可重放、可归因的执行轨迹基础；PR-BL～PR-BR1 已使 Planner、Permission、Authorization、Controller、Execution Agent、Replanner、unified verified inspection、privacy-safe observability 与 Structured Dataset Canary contract 进入 `main`，但仍无代表性 private real-tool runtime `V`。PR #30 的 v3 real-tool runtime bridge 已合入；当前下一 P0 是在 reviewed commit/匹配 worker 上完成 BR1 Uni-Mol applicability preflight，随后才进入最终数据冻结与 clean acceptance。`M3H-014` 保持 `READY`，不在本验收 PR 同时启动；BQ3 与 BR3 继续暂缓。M4 benchmark protocol 继续暂缓，避免抢占 M3.5 integration/runtime closure 的 P0 资源。
...
| `M3H-013` Structured Dataset Canary | `I/T(partial)/—` | `READY` | PR #30 reviewed HEAD `34a4438e31c054f01d77e58b86c14dc21111ccde`、squash merge `a28ea6fe19db9c81e43a40174498c636bd0f1bf8` 的 v3 real-tool runtime bridge、PR #31 merge `19ae10a1c866964d6b993c7dfd100e930127acde` 的 applicability contract、PR #33 merge `5307dd7c446aee21c4d0d026b1e1d94d9b628236` 的 authority materialization，以及 PR #35 actual merge `bf82cfd75d23abfe17328f362bd76b79b134b138` 的 Controller historical-policy compatibility 已合入。新的 post-merge rebind 在实际 merge commit 与 matching worker `sha256:1445ee01fbb3748caea3ddf889726df48ad36d49e0bfb66b3d079d93d6f3aa72`、`unimol-tools 0.1.5`、`unimol-train-br1-v2` 上完成；fresh read-only preflight `PASS`，`1999/1999/0/0`，reason/mapping diagnostics 为空，Raw/canonical/staged/provider-actual digests exact-match；report/summary verifier、authority reread、fixed-time byte replay 通过，training/generation/prediction/ranking/model/checkpoint/scaler/metrics 均未派发或创建。新 Registry-bound candidate package `br1-post-merge-freeze-bf82cfd-v1`（`sha256:d4cf65eea053fb3bd77c2197d6625f4f9859951b580f11a1b20c01625c42e2af`）与 proposal `br1-owner-proposal-bf82cfd-v1`（`sha256:a845be702adc50e7577a685debee22cf37929363e5810421ac93496db1ac11c9`）已生成，状态 `WAITING_OWNER`。现存 approval 只绑定历史 `5307dd7c...`/旧 package/report/summary，不能延伸到本次 identity；当前无 exact-bound owner approval，因此未创建 acceptance ID/run、Permission authorization、review snapshot、Confirmed Dataset 或 Gate；不训练、不生成、不预测、不排名，不标 `V` 或 `DONE`，不推进 BR2。|
...
解锁顺序固定为 `BQ1 → BQ2/BR1/BR2 → BQ3 → BR3`。PR #27 已由 squash merge commit `e9316ad64fd219a26740b3424f541e00a9409d39` 合入 `main`。下一 P0 实现动作为 PR-BR2 / `M3H-014`，当前状态为 `READY`；当前 P0 验收动作为 BR1 Private Real-Tool Canary / `M3H-013`。PR-BQ3 / `M3H-012` 与 PR-BR3 / `M3H-015` final acceptance 保持未启动。M4 继续不得抢占 P0；M5 只允许进行不阻塞 v1 的科学范围与数据充分性准备，不启动 DFT、MD、逆合成、自动文献评分或 Agentic RL。
...
### 2026-08-02：PR #25 合并并启动 PR-BQ2 privacy-safe Harness observability v1

- 决策：PR #25 reviewed HEAD `e10ac49ce6049aa8730766be793c64aee01a8f4d` 已获 repository-owner acceptance，并由 squash merge commit `df64c318ce2576e42da245403c63c4f570facee9` 合入 `main`；`M3H-010` 更新为 `I/T/— / DONE`。PR-BQ2 只实现 `M3H-015` 的 observability 子范围，`M3H-015` 保持 `I(partial)/T(partial)/— / IN_PROGRESS`。
- 原计划：BQ1 完成 unified verified inspection 后按 `BQ2/BR1/BR2 → BQ3 → BR3` 推进；observability 尚只有窄化 Noop/OTel seam，未冻结统一 correlation、LangSmith metadata-only、集中 privacy allowlist 与跨 adapter fail-open 语义。
- 新计划：Molly immutable ledger 继续是 authority，`AgentRunInspection v1` 继续是统一 current-verified 读取边界；OpenTelemetry 与 LangSmith 永远是 non-authoritative telemetry。两者共用 `harness_telemetry_correlation.v1` 和 `harness_telemetry_privacy_policy.v1`，默认无外部 telemetry，启用 LangSmith 时采用 metadata-only；structured content 只有显式 server policy 与数据授权后才能使用，且 v1 不接受 raw prompt/response。
- 依据：新增 optional/lazy OTel HTTP/gRPC 与 LangSmith adapter、bounded composite/noop tracer、process-local non-authoritative health、Planner/Permission/authorization/Controller/local-remote/Execution Agent/Replanner/inspection 接入，以及 dependency-missing、vendor failure、authority-byte/digest invariance、privacy、OLED host terminology、concurrency、restart/hash-seed 与 schema focused tests；observability/tracing focused tests `25 passed`，Controller/local-remote/Execution Agent 相邻链 `60 passed`，LLM provider/Planner/authorization/Execution Agent/Replanner/inspection/privacy 链 `268 passed`，本地 PR Fast `1133 passed, 5454 deselected`，`compileall`、`git diff --check` 与 4-shard assignment validation 通过。GitHub 4-shard Full CI 是完整套件的权威 evidence。
- 影响任务：PR-BQ2 完成后仍不关闭整个 `M3H-015`；最终 UI-driven runtime、restart、recovery、exact replay、privacy 与 adversarial evidence 继续由 PR-BR3 完成。BR1、BR2、BQ3 未在本 PR 启动；不新增任何 Gate `V`，不声明 M3.5 或 Molly v1 完成。
...
### 2026-08-02：PR #26 合并并启动 PR-BR1 Structured Dataset Canary v1

- 决策：PR #26 reviewed HEAD `ba574eb69bb204de5f8ee1b776a0aebff64d5fa9` 已完成复核，并由 squash merge commit `0c558a5e1a029a7e715048e11c6edcc0ab76c40e` 合入 `main`；`M3H-010` 保持 `I/T/— / DONE`，`M3H-015` 保持 `I(partial)/T(partial)/— / IN_PROGRESS`。当前唯一实现动作更新为 PR-BR1 / `M3H-013`，其状态更新为 `I/T(partial)/— / IN_PROGRESS`。
- 原计划：PR-BQ2 完成 observability seam 后，BR1、BR2 可分别实现两条 canary，随后才由 BQ3 与 BR3 完成 UI 和最终 acceptance。
- 新计划：BR1 只实现 Structured Dataset Canary 的 production-equivalent Raw/Confirmed Dataset、exact confirmation、fresh model、generation、prediction/ranking、chemical validation、recovery/replay、inspection、observability 与 privacy contract；公共 CI 完成 deterministic reference canary，私有真实环境入口和 evidence contract 同时冻结。PR-BR2 / `M3H-014`、PR-BQ3 / `M3H-012` 与 PR-BR3 / `M3H-015` final acceptance 继续保持未启动。
- 依据：PR #26 reviewed HEAD `ba574eb69bb204de5f8ee1b776a0aebff64d5fa9`；PR #26 squash merge `0c558a5e1a029a7e715048e11c6edcc0ab76c40e`。当前公开环境不具备可声明 fresh Uni-Mol 与 real REINVENT4 成功的 owner-reviewed private runtime evidence。
- 影响任务：`M3H-013` 仅进入 `I/T(partial)/— / IN_PROGRESS`；不新增任何 `M3H-GATE-* V`，不标记 `M3H-015 DONE`、M3.5 完成或 Molly v1 完成。即使 CI Reference Canary 通过也不产生 runtime `V`。
...
### 2026-08-03：PR #27 合并并启动 BR1 Private Real-Tool Canary preflight

- 决策：PR #27 reviewed HEAD `e7bb7fc45f0c514ccce000d7fff25ee66e967bd6` 已完成复核，并由 squash merge commit `e9316ad64fd219a26740b3424f541e00a9409d39` 合入 `main`。下一 P0 实现动作调整为 PR-BR2 / `M3H-014`；当前 P0 验收动作调整为 BR1 Private Real-Tool Canary / `M3H-013`。
- 原计划：PR-BR1 同时冻结 CI Reference Canary 与 private real-tool request/runbook，待合并后在可信私有环境执行 fresh Uni-Mol、real REINVENT4、restart 与 exact replay 验收。
- 新计划：严格复用 PR #27 已进入 `main` 的唯一 Harness authority chain 执行 private canary；preflight 必须先证明 server-owned Uni-Mol 与 REINVENT4 resource authority 完整可用。当前环境虽有两个 logical execution profile 与 current available capability probe，但缺少 server-owned remote resource authority policy，因此 `M3H-013` 暂标 `BLOCKED`，不创建正式 acceptance manifest/run，不启动 Controller、training 或 generation，也不降级到 fake provider。
- 依据：PR #27 reviewed HEAD `e7bb7fc45f0c514ccce000d7fff25ee66e967bd6`；PR #27 squash merge `e9316ad64fd219a26740b3424f541e00a9409d39`；2026-08-03 privacy-safe resource preflight 结论为 `resource_authority_policy_configured=false`。
- 影响任务：`M3H-013` 保持 `I/T(partial)/—` 且因明确 blocker 标记 `BLOCKED`；`M3H-014` 为下一 P0 实现动作但尚未启动，保持 `I/T(partial)/— / READY`。不新增任何 `M3H-GATE-* V`，不标记 `M3H-015 DONE`、M3.5 完成或 Molly v1 完成。
- 新增风险：在 policy 缺失时绕过 Controller/RemoteExecutionService、硬编码 SSH、降级为 local fake 或伪造 real-tool evidence；私有连接与数据定位信息进入公共 evidence。
- 批准人：待 repository owner 配置 resource authority policy，并对后续完整 clean acceptance exact HEAD review。

### 2026-08-03：PR #28 blocked-preflight evidence/governance review

- 决策：PR #28 只作为 fail-closed blocked-preflight 记录并继续保持 Draft；不得占用正式 `structured_dataset_private_real_tool_evidence.v1` schema，不得从本 preflight ID 启动或拼接后续 acceptance。
- 原计划：公开 evidence 只记录缺少 remote resource authority policy 与 Raw Dataset contract 不完整，并把尚未启动的 PR-BR2 / `M3H-014` 标为 `IN_PROGRESS`。
...
- 实现边界：新增 normalized condition、source-aware observation identity、source-independent conflict group 与 v2 confirmation binding；target value 仍是 payload，不进入 identity；split 仍使用 InChIKey–paper 二部图 connected components。v2 prepare 在 normal completion 与 crash reconstruction 均读取 exact Raw rows，使用同一 molecule inspector 和冻结时间重建完整 expected review，并对 molecule、condition、target payload、duplicate/conflict findings、reason codes、actions 与 rosters 执行 exact comparison；confirmation 前再次执行同一 derivation，不能以嵌套 digest 自洽替代 Raw-row 科学语义绑定。mapping policy 的 owner authority 来自绑定其 digest 的 trusted plan authorization，不来自客户端布尔字段。server-owned remote resource authority 的通用 operator 配置方法写入 README，但 policy 不得把 local Planner route 变成 remote route。
...
- 批准人：repository owner 已完成 PR #29 exact-HEAD review；合并事实见下一条决策记录。

### 2026-08-03：PR #29 合并并复核 BR1 real-tool runtime readiness

- 决策：PR #29 reviewed HEAD `191632b3b79b3f470a0c3f4f28254413bee72086` 已完成 repository-owner exact-HEAD 复核，并由 squash merge commit `7f974c9ae92e72830ebe4ec0a7f345354b996167` 合入 `main`。PR Fast、CodeQL 与 4-shard Full CI 已通过；private v2 source/mapping authority、condition-aware observation identity 与 derivational review verifier 成为当前基线。
- 现状：server-owned logical resource authority policy 已存在并与 `unimol-train-v1`、`reinvent4-cpu-v1` provider profile 匹配，历史 blocked-preflight evidence 保持不可变；但当前 private registry 仍只版本化 dataset preparation，后续 Structured Dataset tasks 仍路由 CI ridge/deterministic generator/local prediction。现有 Uni-Mol worker 训练输出仅发布孤立 checkpoint，不能满足官方 `MolPredict(load_model=<model directory>)` 的 current-model prediction contract。
- 新计划：先实现最小、版本化的 BR1 real-tool runtime bridge，保持默认 v1 与 private v2 exact replay 不变；随后按合入 schema 重签私有 dataset manifests/CSV，完成 provider applicability preflight，最后才创建新的 acceptance ID/run ID 启动正式 Controller run。不得在 bridge、数据 authority 与 applicability 全部通过前 dispatch Uni-Mol 或 REINVENT4。
- 影响任务：`M3H-013` 继续 `I/T(partial)/— / BLOCKED`；`M3H-014` 继续 `I/T(partial)/— / READY`。本 correctness follow-up 不产生 runtime `V`，不新增 `M3H-GATE-006 V`，不关闭 M3.5、`M3H-015` 或 Molly v1。
- 新增风险：为 real-tool acceptance 直接修改默认 catalog、复用 v1 task IDs 或只发布不可预测的孤立 checkpoint，会破坏历史 exact replay 或造成“真实训练但非 current-model prediction”的竞争事实。
- 批准人：runtime bridge Draft PR 仍需 repository-owner exact-HEAD review；不得自动转 Ready 或合并。

### 2026-08-03：BR1 Uni-Mol applicability preflight v1 作为下一 P0

- 决策：PR #30 reviewed HEAD `34a4438e31c054f01d77e58b86c14dc21111ccde` 已通过复核，并由 squash merge commit `a28ea6fe19db9c81e43a40174498c636bd0f1bf8` 合入 `main`；v3 real-tool runtime bridge 已成为当前 BR1 runtime 基线。本 Draft PR 新增独立、版本化、只读、fail-closed 的 Uni-Mol provider applicability preflight，不修改 runtime task graph、Planner、Permission、Controller 或冻结的 source/mapping schema。
- 原计划：在 bridge 合入后直接重冻结私有数据并创建 clean BR1 acceptance，随后执行 real-tool training、generation、prediction/ranking 与 restart/replay。
- 新计划：先在 reviewed commit 和匹配 `molly-worker` 上对 exact Raw CSV、`source_dataset_manifest.v1`、`br1_raw_dataset_mapping_policy.v1` 和实际 `unimol-train-br1-v2` provider 环境运行 applicability preflight；只有 `PASS` 才能最终冻结数据并创建 acceptance。`REVIEW_REQUIRED` 必须等待 owner 的显式 exclusion decision；`BLOCKED` 不得启动 acceptance。当前 preflight 是下一 P0，但不构成 runtime `V`。
- 依据：当前 worker 的 Uni-Mol training path 使用 `unimol-tools` `MolTrain` 并在 `fit()` 前创建训练输出；仓库无法证明该 provider 暴露只读 preprocessing API。因此 preflight 默认拒绝猜测 provider API，不能用短 epoch training、临时模型或 checkpoint 代替 applicability evidence。
- 影响任务：`M3H-013` 保持 `I/T(partial)/— / BLOCKED`；`M3H-014` 保持 `I/T(partial)/— / READY`；不新增 Gate `V`、正式 acceptance ID/run ID、M3.5 `DONE` 或 Molly v1 完成声明。
- 新增风险：真实 provider 的 preprocessing API、元素/电荷/原子数边界和构象生成语义仍需在匹配环境中明确；任何 unsupported/unresolved 行都不能静默转为 excluded 或被自动批准。公共 summary 只能包含 counts、reason counts、safe digests、provider/profile 和 claim boundary，不得包含 row ID、SMILES、路径、主机、用户名、命令或 stdout/stderr。
- 批准人：待 repository owner 对本 Draft PR exact HEAD 复核；合并后必须在 reviewed commit 上重新运行，不记录本 PR 的 smoke/preflight 为 acceptance。

### 2026-08-03：BR1 remote preflight remediation v1

- 决策：在 PR #31 合并基线 `19ae10a1c866964d6b993c7dfd100e930127acde` 上建立独立 Draft remediation PR，处理上一轮远端 applicability preflight 的五个 blocker；不修改 runtime task graph、Planner、Permission、Controller 或冻结的 v1/v2/v3 runtime authority。
- 原计划：上一轮 preflight 失败后直接等待数据重冻结和 clean acceptance。
- 新计划：先闭合 `authorized source artifact → canonical source reconstruction → server-owned mapping binding → canonical provider input → staged bytes → provider actual bytes` identity chain；增加 source publication registry/authority contract、exact mapping binding、`unimol-tools 0.1.5` 的 project-owned read-only capability/DataHub adapter、逐行三态分类、report identity fields、no-dispatch assertions 与 forged authority/report/summary adversarial tests。然后在匹配 worker/profile 的远端 fresh run 上重新验证；若 authority 仍不完整，继续 `BLOCKED`。
- 依据：上一轮远端 reviewed commit 为 `19ae10a1c866964d6b993c7dfd100e930127acde`，provider `unimol-tools 0.1.5`、profile `unimol-train-br1-v2`，结果为 `BLOCKED`、`1999/0/0/1999`，report digest `sha256:f47b6a50d4fa73f6bfcf2384918e484fc2a46d38c68a93b24d1ea1a66c7f17db`，reasons 为 `INPUT_DIGEST_MISMATCH`、`SOURCE_AUTHORITY_INVALID`、`MAPPING_POLICY_INVALID`、`PROVIDER_PREFLIGHT_API_UNAVAILABLE`、`PROVIDER_CAPABILITY_UNAVAILABLE`。该结果未创建 acceptance ID/run，未派发 training/generation/prediction/ranking。
- 新远端证据：在 remediation execution HEAD `c23b7a0eb8897b0d65bd33b4763eb8f8815f46f7`、匹配 worker、`unimol-tools 0.1.5`、`unimol-train-br1-v2` 上 fresh run 产出 `BLOCKED`、`1999/0/0/1999`；report digest 为 `sha256:e86467ab217f99d8a755b963d1bb46adcb5ddfdb7d736f4819ae818d72e6901e`，summary digest 为 `sha256:57443e3f4c5767ef579af931299d029eb0d69b5ce948b073eb3af785cc17da86`。report contract 和 summary projection 均验证通过；provider capability probe 已发现，剩余 reasons 为 `INPUT_DIGEST_MISMATCH`、`MAPPING_POLICY_INVALID`、`SOURCE_AUTHORITY_INVALID`、`SOURCE_PUBLICATION_REGISTRY_INVALID`。由于 authority fail-closed，未执行逐行 provider preprocessing；所有 training/generation/prediction/ranking 和模型产物断言均为 false。
- 影响任务：`M3H-013` 保持 `I/T(partial)/— / BLOCKED`；`M3H-014` 保持 `I/T(partial)/— / READY`。本 remediation 只推进 pre-acceptance readiness，不标记 `M3H-013` 为 `V`/`DONE`，不启动 BR2，不记录新的 owner acceptance。
...
### 2026-08-04：PR #35 合并后的 BR1 authority rebind 与 acceptance readiness

- 决策：PR #35 已由 repository owner 合并，实际 merge commit 为 `bf82cfd75d23abfe17328f362bd76b79b134b138`。从该最新 `main` 建立 clean follow-up 分支，并在匹配 worker/provider 环境中重新 materialize 全套 source authority；不复用 PR #33/旧 PR #35 的 authority、report、freeze 或 proposal。
- 新远端 authority：matching worker implementation digest 为 `sha256:1445ee01fbb3748caea3ddf889726df48ad36d49e0bfb66b3d079d93d6f3aa72`；provider 为 `unimol-tools 0.1.5`；profile 为 `unimol-train-br1-v2`，profile digest 为 `sha256:8d700004761bcac419ef853828607a4ebad47a88892be22624ec551e52246ac5`。新的 source manifest digest 为 `sha256:4657ce19dbeb0b433eadfc98b664c085c19e2778fd001035c8c0e17e2f044a36`，mapping policy digest 为 `sha256:0f58c28e37f436c77fd167577cdc88e0966fcd816e5afa1e0b94e314b63a6912`，source materialization binding digest 为 `sha256:9fb6f0ab8e61fbcd35cc3f7649cafe4503465479f663d39f05c64b04446ae759`，publication digest 为 `sha256:689258012a1f3530928bd3d93c610a408446a9a1673b6f5001523fe4697a7d9b`，registry digest 为 `sha256:f102fc53931540748af361b6f08dea3851b6964ba2f5a7ab102ba485a1ad8dbc`，authority semantic digest 为 `sha256:20d4ba17e9706eda7f38f03d94e14c9dd1e240557393754dee1a17f60b2553ac`，authority file digest 为 `sha256:27728342555a8215f92ac40e72a1608867b7ccce068388731c3de8604ce135da`。
- 稳定 identity：Raw digest `sha256:755c8bb312c25deffb7bba4a77904e8337646959ecc802575444b2620f848efa`、canonical source digest `sha256:817d936c343fd63edc50acc85472a2c407df9c60d9d34884bc7f2228b8aab85c`、canonical provider digest `sha256:d8770caf126c68c5b81788d256b985e7d72d60b6bfcbc7b82ca3fc790d8e2da5` 与历史 PASS exact-match；输入行数 `1999`。
- 新鲜 preflight：`PASS`，`SUPPORTED=1999`、`UNSUPPORTED=0`、`UNRESOLVED=0`，`reason_counts={}`、`mapping_diagnostics={}`；report digest 为 `sha256:24f431dc201441594858f316fd20980a75b414c6a0e8766edd02fc60e52ce3e8`，summary semantic digest 为 `sha256:4f04b9e9cba2d88495e0b052f0a6926da6c95d61e673fe826cabca7e17ce8f70`。expected/observed Raw 及 observed/staged/provider-actual canonical provider digest exact-match；report contract、summary projection、authority chain reread/schema/cross-binding 与固定时间 report/summary byte replay 全部通过。
- 安全边界：capability probe 与 provider preprocessing 为 true；training、generation、prediction、ranking、model artifacts、scaler、training metrics 均为 false，未创建科学任务产物。新 private staging/freeze package 使用受限目录、regular non-symlink 文件与 `0400/0600` 权限；private report 未进入 Git。
- 冻结与 proposal：exact Raw CSV、source manifest、mapping policy 已复制为 Registry-bound package `br1-post-merge-freeze-bf82cfd-v1`，package digest 为 `sha256:d4cf65eea053fb3bd77c2197d6625f4f9859951b580f11a1b20c01625c42e2af`；privacy-safe proposal 为 `br1-owner-proposal-bf82cfd-v1`，proposal digest 为 `sha256:a845be702adc50e7577a685debee22cf37929363e5810421ac93496db1ac11c9`，当前 `WAITING_OWNER`。proposal 明确声明 preflight PASS/freeze 不等于 data quality、confirmation Gate、training authorization 或 BR1 completion。
- Owner 边界：发现的 trusted approval exact-bind 历史 PR #33 merge、旧 freeze、旧 report/summary，和当前 proposal 的 commit/package/report/summary 不一致，因此 fail closed；本轮没有有效 exact-bound owner approval。本 prompt、PASS、freeze、CI 与 Codex 输出均不构成 approval。
- 影响任务：`M3H-013 = I/T(partial)/— / READY`；已完成 post-merge preflight authority，已冻结 acceptance candidate inputs，等待 exact-bound owner decision；无 acceptance ID/run、无 Permission authorization、无 Controller action、无 review snapshot、无 Confirmed Dataset、无 human Gate；不启动训练、生成、预测、排名，不标 `V`/`DONE`，不推进 BR2。
- 批准人：待 repository owner 对 `br1-owner-proposal-bf82cfd-v1` 做明确 `ACCEPT_EXACT_PROPOSAL` 或拒绝决定；approval 必须 exact-bind 当前 merge commit、Raw/manifest/mapping/report/summary/freeze package identity。
...
### 下一 P0 实现 follow-up：BR1 post-merge authority rebind and acceptance readiness v2（本 Draft PR）
...
1. Raw Dataset 永不成为 training authority；training 必须 exact-bind GateDecision、confirmation receipt 与 Confirmed Dataset publication；
2. baseline model 在当前 run 真实 fit，generation 真实执行确定性逻辑，prediction/ranking 只消费当前 model/candidate roster，旧输出与 `existing_output` fail closed；
3. chemical validation 覆盖 identity、duplicate、training exact match、similarity、scaffold、AD/OOD 与 no-silent-loss；
4. restart/recovery 不重复 training 或 generation dispatch，exact replay 与 telemetry availability 无关；
5. 公共 evidence 保持脱敏，输出只称 `Computational Top-N`；本 PR 不声明 private runtime、Gate `V`、BR2、BQ3、BR3、M3.5 或 Molly v1 完成。
