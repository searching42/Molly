...
The demo runner can also load a local JSON input bundle of scenario summaries, allowing low-risk local review of user-supplied OLED demo payloads without reading referenced artifacts or executing adapters.
...
A CLI entrypoint, `ai4s_agent.agents.oled_local_demo_runplan`, runs the low-risk OLED local demo through `RunPlanExecutor` while preserving the no-scientific-adapter safety boundary.
...
A bounded worker-loop CLI, `ai4s_agent.agents.oled_local_demo_worker_loop`, can consume existing OLED local demo queue jobs without starting a daemon or executing scientific adapters.

`ai4s_agent.agents.oled_local_demo_enqueue` can enqueue an OLED local demo worker job without executing it, enabling a two-step queue-submit then bounded-worker-loop flow.

`ai4s_agent.agents.oled_local_demo_retry` can enqueue a retry child for a failed OLED local demo worker job without executing it, enabling explicit recovery before the bounded worker loop consumes the retry.
...
`ai4s_agent.agents.oled_local_demo_health` provides a read-only health check for the OLED local demo worker queue stack before submitting or consuming jobs.
...
`ai4s_agent.agents.oled_local_demo_generic_status` provides a read-only view of generic `run_plan_execute` OLED local demo jobs, leases, and optional ProjectStorage metadata.

`ai4s_agent.agents.oled_local_demo_generic_retry` queues a retry child for failed allowlisted generic `run_plan_execute` OLED local demo jobs without executing it.

`ai4s_agent.agents.oled_local_demo_generic_cancel` cancels queued allowlisted generic `run_plan_execute` OLED local demo jobs or requests cancellation for running jobs without executing adapters.
