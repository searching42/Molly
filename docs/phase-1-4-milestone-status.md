...
| Phase 4 queued execute canary | Completed as feature-flagged, allowlisted, rollout-policy documented, first artifact registry parity fixture started, first failure classification parity fixture started, repeated-run stability coverage started, queue recovery/stale lease coverage started, default-migration readiness checklist documented, and second allowlisted chain parity started; not default migrated | `tests/test_run_plan_executor.py`, `tests/test_queued_execute_canary_artifact_parity.py`, `tests/test_queued_execute_canary_failure_parity.py`, `tests/test_queued_execute_canary_second_chain_parity.py`, `tests/test_queued_execute_canary_repeated_run_stability.py`, `tests/test_queued_execute_canary_queue_recovery.py`, `tests/test_queued_execute_canary_default_migration_readiness_docs.py`, `docs/queued-execute-canary-rollout-policy.md` |
...
11. Repeated-run stability coverage has started for existing allowlisted queued
...
12. Remaining canary migration work includes cancellation/retry coverage, a
    production-sized fixture, the default migration decision, remote worker
