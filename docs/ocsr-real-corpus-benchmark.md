...
The report contract re-derives all counts and rates, per-paper summaries,
confidence summaries, candidate artifact/result bindings, document-paper role
coverage, and its report digest during validation.

## Corpus-size claim

The report is a `bounded_real_paper_canary` until it contains at least three
papers and twenty source diagrams. Only a report satisfying both thresholds is
labeled `real_corpus_benchmark`. These labels prevent a successful harness run
or a small canary from being reported as broad scientific accuracy evidence.
...
revalidation. Cleanup only removes the invocation-owned inode.
