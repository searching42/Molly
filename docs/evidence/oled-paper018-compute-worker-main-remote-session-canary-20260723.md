# Paper018 logical compute-worker-main remote REINVENT4 session canary — 2026-07-23

> **Sanitized legacy-private evidence.** Resource names in this summary are
> logical public identifiers. Concrete SSH aliases, hostnames, usernames,
> known-hosts material, interpreter paths, runtime directories, and raw bundles
> remain in the private audit environment. Unlinked PR numbers refer to the
> pre-migration private archive.

## Scope

This canary exercised the complete bounded discovery session through the PR-AW
HTTP control plane with a real remote PR-AS generation through the logical
`compute-worker-main` resource. The run used the retained compatibility profile
identifier `molly-compute-main-compute_worker_main-reinvent4-v1`, then locally
exact-replayed the remote publication through PR-AT controlled prediction,
PR-ARb v2 Top-N selection, and PR-AU termination. The compatibility identifier
is not a public hostname or connection instruction.
...
The ignored raw runtime tree remains in the private audit environment. Its
locator is intentionally omitted; the table preserves only reviewed identities
and safe content digests.

| Input | Exact identity | SHA-256 / digest |
|---|---|---|
| Molly code | commit `fe2328c` on top of merged legacy-private PR 391 | commit identity |
...
| pinned host-verification anchor | private locator and bytes omitted | binding verified; digest retained only in the private execution record |

The logical remote environment used REINVENT 4.7.15 from source commit
`5e67f40eedbb4c617710f8156b1db585cd789770`. The private interpreter path is
omitted. The prior SHA-256 was
...
Immediately before approval, a private resource preflight confirmed adequate
capacity and no conflicting workload. Exact machine telemetry is omitted. The
transport used the profile's `cpu_only` and `nice_19_single_thread` policies,
so the canary did not allocate GPU memory.
...
`endpoint_hostname_verified=true`; the expected hostname and invocation-isolated
remote attempt directory are private locators and are omitted here. No automatic
remote cleanup or process manipulation was performed.
...
The sanitized project label `paper018-remote-reinvent4-canary-20260723` reached
...
The logical `compute-worker-main` remote transport and the complete single-round
bounded discovery session are accepted as an executable canary. The persisted
claims remain
