# GPU worker MinerU Endpoint Preflight Evidence - 2026-06-28

This document records a redacted evidence summary for a manual MinerU endpoint
preflight run on gpu_worker_main. It intentionally does not include the full
...
| Field | Value |
| --- | --- |
| Run ID | `mineru-preflight-gpu_worker_main-20260628-144842` |
| Date | 2026-06-28 |
| Generated at | `2026-06-28T06:48:42.982175Z` |
| Node | `gpu_worker_main` |
| SSH target used | `molly-gpu-main` (hostname reported as `gpu_worker_main`) |
...
The run used the PR #150 preflight source synchronized to a temporary gpu_worker_main
directory because gpu_worker_main did not have a Molly checkout at the time of evidence
...
The complete preflight artifact bundle was retained outside git on gpu_worker_main:

```text
/tmp/molly-mineru-preflight/mineru-preflight-gpu_worker_main-20260628-144842/
```

A tarball was created outside git for bundle integrity tracking:

```text
/tmp/molly-mineru-preflight/mineru-preflight-gpu_worker_main-20260628-144842.tar.gz
```
