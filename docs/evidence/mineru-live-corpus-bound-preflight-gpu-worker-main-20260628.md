# GPU worker MinerU Preflight-Bound Live Corpus Acceptance Evidence - 2026-06-28

This document records a redacted evidence summary for a manual preflight-bound
MinerU live corpus acceptance run against the gpu_worker_main self-hosted endpoint. It
...
```text
gpu_worker_main endpoint preflight
...
The endpoint profile was `gpu_worker_main-loopback` through the `manual-primary`
...
| Field | Value |
| --- | --- |
| Run ID | `mineru-preflight-gpu_worker_main-20260628-154527` |
| Generated at | `2026-06-28T07:45:27.957941Z` |
| Node | `gpu_worker_main` |
| Decision | `passed` |
| Health status | `healthy` |
| MinerU version | `3.4.0` |
| Protocol version | `2` |
| Endpoint profile | `gpu_worker_main-loopback` |
| Routing policy | `manual-primary` |
| Redacted API origin | `http://127.0.0.1:18000` |

The preflight was executed on gpu_worker_main using the same module entry point from a
...
GPU worker environment summary recorded by the preflight:
...
- `gpu_worker_main_hint_vllm_use_flashinfer_sampler`
- `gpu_worker_main_hint_ld_library_path_cuda_cudnn_ordering`

The full preflight artifact bundle was retained outside git:

```text
/tmp/molly-mineru-preflight/mineru-preflight-gpu_worker_main-20260628-154527/
```

Preflight artifact tarball retained outside git:

```text
/tmp/molly-gpu_worker_main-remote-preflight/mineru-preflight-gpu_worker_main-20260628-154527.tar.gz
```
...
The live corpus acceptance was run with the gpu_worker_main preflight report as a hard
pre-parse binding:
...
PYTHONPATH=src python \
  ...
  --run-id mineru-corpus-live-bound-20260628-154606 \
  --preflight-report /tmp/molly-gpu_worker_main-remote-preflight/mineru-preflight-gpu_worker_main-20260628-154527/preflight_report.json \
...
| Bound preflight run id | `mineru-preflight-gpu_worker_main-20260628-154527` |
