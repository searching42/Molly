...
The first known-good reference instance is gpu_worker_main, but the process is not
gpu_worker_main-specific. Operators can reuse the same gate on other GPU hosts when
...
- GPU worker evidence under `docs/evidence/` is the first known-good reference
  instance, not a requirement that other operators use gpu_worker_main.
