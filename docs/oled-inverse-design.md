...
`remote` mode is restricted to the checked-in compatibility policy identifiers
`molly-gpu-main-gpu_worker_main-reinvent4-v2` and
`molly-compute-main-compute_worker_main-reinvent4-v1`. These stable strings are
policy and replay keys, not live hostnames or connection instructions. Each key
binds a logical connection and environment ID that resolves through private
user-level configuration. Both policies require an executor-frozen
`oled_inverse_design_remote_known_hosts` artifact and use
`StrictHostKeyChecking=yes`. Before creating a remote work directory, the
strict SSH session verifies the expected hostname from that private connection
profile.

Arbitrary SSH hosts, repositories, Python paths, and conda environments are
not task options. Their concrete values come from the bound private profiles.
Both executable policies are deliberately CPU-only. Each launches one
