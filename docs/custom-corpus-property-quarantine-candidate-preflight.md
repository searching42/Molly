...
## After Quarantine Candidate Preflight: Training Admission Readiness

The downstream readiness planner is documented in:

```text
docs/custom-corpus-property-training-admission-readiness.md
```

Future evidence can use:

```text
docs/evidence/templates/custom-corpus-property-training-admission-readiness-evidence-template.md
```

Quarantine candidate preflight checks candidate-only artifacts. Training
admission readiness determines whether those candidates can be considered by a
future training admission request. Training admission remains separate: the
readiness planner does not create a request, admit training data, create
training or candidate CSV/JSONL/Parquet/LMDB artifacts, run Phase 1, or modify
`DatasetConfirmation`.
