...
def _next_action(
    *,
    ...
    spec: dict[str, Any],
    state: dict[str, Any],
    materialize_execution_inputs: bool = True,
) -> dict[str, Any]:
    ...
    if step == SCREENING:
        return _action_for_label(
            ...
            state,
            "screening",
            materialize_execution_inputs=materialize_execution_inputs,
        )
    ...
    if round_index > 1:
        roster_inputs["oled_inverse_design_generation_roster"] = str(
            _generation_roster(
                ...
                state,
                round_index,
                materialize=materialize_execution_inputs,
            )
        )
...
def _controller_request(
    ...
    *,
    ...
    _materialize_or_verify_immutable_bytes(
        path,
        _json_bytes(payload),
        materialize=materialize,
    )
...
def _validate_external_state(
    ...
    state: dict[str, Any],
    *,
    allow_waiting_child_ahead: bool = False,
    materialize_execution_inputs: bool = True,
) -> dict[str, Any] | None:
    ...
    reconciliation_available: dict[str, Any] | None = None
    for child in state["children"]:
        ...
        if child_status == "waiting_user":
            if stage.status != RunStatus.WAITING_USER:
                if allow_waiting_child_ahead and stage.status in {
                    RunStatus.SUCCEEDED,
                    RunStatus.FAILED,
                    RunStatus.RUNNING,
                }:
                    reconciliation_available = {
                        "available": True,
                        "session_revision": int(state["revision"]),
                        "run_id": run_id,
                        "task_id": task_id,
                        "session_child_status": child_status,
                        "observed_stage_status": stage.status.value,
    ...
    if status == WAITING_USER:
        ...
        return reconciliation_available
    if waiting_labels:
...
def _materialize_or_verify_immutable_bytes(
    path: Path,
    payload: bytes,
    *,
    materialize: bool,
