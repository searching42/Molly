...
def _install_modeling_plan_route_split() -> None:
    ...
    def register_routes_with_external_approval_split(
        app: Any,
        base_runs_dir: Any = None,
        workspace_dir: Any = None,
        **kwargs: Any,
    ) -> None:
        storage_count = len(_PROJECT_STORAGES)
        original_register_routes(
            app,
            base_runs_dir=base_runs_dir,
            workspace_dir=workspace_dir,
            **kwargs,
        )
        storage = _PROJECT_STORAGES[-1] if len(_PROJECT_STORAGES) > storage_count else _PROJECT_STORAGES[-1]
        app.view_functions["agent_modeling_plan"] = _make_agent_modeling_plan_with_split(storage)
...
def _make_agent_modeling_plan_with_split(storage: Any):
    def agent_modeling_plan_with_split():
    ...
    return agent_modeling_plan_with_split
