...
class OledBoundedDiscoverySessionActionService:
    ...
    def list_session_actions(
        ...
        """Return validated action telemetry without changing coordinator state."""

        clean_project = validated_oled_bounded_project_id(project_id)
        clean_session = str(session_id or "").strip()
        records: list[dict[str, Any]] = []
        candidate = self.actions_root / clean_project
        if candidate.is_symlink():
            raise ValueError("PR-AW project action root is a symbolic link")
        root = candidate.resolve()
        if not root.is_relative_to(self.actions_root):
            raise ValueError("PR-AW project action root escapes storage")
        if not root.exists():
            return []
        if not root.is_dir():
            raise ValueError("PR-AW project action root is invalid")
        for action_dir in sorted(root.glob("oled-session-action-*")):
            ...
            request, state, _ = self._read(clean_project, action_dir.name)
            ...
            if status in _ACTIVE and not self._owned_active_action(state):
                ...
                records.append(_public_record(request, state))
        return records
