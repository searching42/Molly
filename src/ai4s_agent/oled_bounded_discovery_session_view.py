...
def build_oled_bounded_discovery_session_view(
    *,
    ...
    with _session_lock(session_dir):
        view, _ = _build_locked_view(storage, clean_project, session_dir)
        return view


def replay_oled_bounded_discovery_projection_source(
    *,
    storage: ProjectStorage,
    project_id: str,
    session_id: str,
) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any] | None]:
    """Return a strictly read-only snapshot and immutable revision history."""

    clean_project = validated_oled_bounded_project_id(project_id)
    session_dir = _session_dir(storage, clean_project, session_id)
    with _session_lock(session_dir):
        spec = _read_spec(session_dir)
        states = _read_immutable_state_history(session_dir)
        state = states[-1]
        reconciliation = _validate_external_state(
            storage,
            clean_project,
            session_dir,
            spec,
            state,
            allow_waiting_child_ahead=True,
            materialize_execution_inputs=False,
        )
        view = _render_locked_view(
            storage,
            clean_project,
            session_dir,
            spec,
        ...
        view["reconciliation"] = reconciliation or {"available": False}
        return view, states, reconciliation


def _build_locked_view(
    storage: ProjectStorage,
    project_id: str,
    session_dir: Path,
    ...
    return _render_locked_view(storage, project_id, session_dir, spec, state), state


def _read_immutable_state_history(session_dir: Path) -> list[dict[str, Any]]:
    names = sorted(
        path.name
    ...
    if not names or names != [f"state_{index:06d}.json" for index in range(len(names))]:
        raise ValueError("PR-AW projection source revision history is incomplete")
    states: list[dict[str, Any]] = []
    previous: dict[str, Any] | None = None
    for revision, name in enumerate(names):
        item = _validated_state_payload(
            _read_session_json(session_dir, name),
            session_dir=session_dir,
            expected_revision=revision,
        )
        _validate_state_child_structure(item)
        if previous is None:
            if item.get("previous_state_digest") is not None:
                raise ValueError("PR-AW projection source initial predecessor is invalid")
        else:
            if item.get("previous_state_digest") != previous["state_digest"]:
                raise ValueError("PR-AW projection source revision chain is invalid")
            _validate_state_transition(previous, item)
        states.append(item)
        previous = item
    return states
    ...
    view = {
        ...
        "remaining": {
            ...
            "generated_candidates": max(
                0,
                limits["max_generated_candidates"] - usage["generated_candidates"],
            ),
        },
        "children": [
            {
