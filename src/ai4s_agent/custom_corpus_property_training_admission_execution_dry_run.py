...
_SCHEMA_VERSION = "custom_corpus_property_training_admission_execution_dry_run.v1"
...
class CustomCorpusPropertyTrainingAdmissionExecutionDryRunError(ValueError):
    pass


def dry_run_property_training_admission_execution(
    *,
    ...
    dry_run_id: str,
    created_by: str,
    confirm_training_admission_execution_dry_run: bool = False,
    ...
    errors = _consistency_errors(
        ...
        run_dir=run_dir,
        confirm_training_admission_execution_dry_run=confirm_training_admission_execution_dry_run,
    ...
    dry_run_status = "blocked" if errors else "needs_review" if warnings else "passed"
    dry_run_records = _dry_run_records(request, hashes, dry_run_id)
    ...
    markdown = _evidence_markdown(report)
    if _contains_forbidden_material({"report": report, "markdown": markdown}):
        return _minimal_redaction_failure()

    report_path = run_dir / "property_training_admission_execution_dry_run_report.json"
    evidence_path = run_dir / "redacted_property_training_admission_execution_dry_run_evidence.md"
    ...
    *,
    ...
    try:
        report = dry_run_property_training_admission_execution(
            ...
            confirm_training_admission_execution_dry_run=args.confirm_training_admission_execution_dry_run,
        ...
        err.write(f"property training admission execution dry-run invalid: {_safe_exception_message(exc)}\n")
        return 1
    output.write(json.dumps(report, ensure_ascii=False, sort_keys=True))
    output.write("\n")
    return 1 if report.get("dry_run_status") == "blocked" else 0


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m ai4s_agent.custom_corpus_property_training_admission_execution_dry_run",
        description="Dry-run a property training admission execution request package.",
    )
    ...
    parser.add_argument("--confirm-training-admission-execution-dry-run", action="store_true")
...
def _consistency_errors(
    *,
    ...
    created_by: str,
    run_dir: Path,
    confirm_training_admission_execution_dry_run: bool,
    allow_execution_preflight_needs_review: bool,
    minimum_dry_run_records: int,
    warnings: list[str],
) -> list[str]:
    errors: list[str] = []
    if not confirm_training_admission_execution_dry_run:
        errors.append("confirmation_required")
    if not _is_safe_id(dry_run_id):
    ...
    _append_errors(
        ...
        _status_errors(
            ...
            quarantine_preflight,
            quarantine_candidate,
            allow_execution_preflight_needs_review,
            warnings,
        ),
    )
    _append_errors(
...
def _record_errors(
    ...
    if len(execution_records) < max(minimum_dry_run_records, 1):
    ...
    if int(execution_preflight.get("planned_candidate_count", len(request_planned_ids))) != len(request_planned_ids):
...
def _dry_run_records(request: dict[str, Any], hashes: dict[str, str], dry_run_id: str) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    for record in _safe_records(request.get("execution_records")):
        execution_record_id = str(record.get("execution_record_id", ""))
        records.append(
            {
                "dry_run_record_id": f"{dry_run_id}-{execution_record_id}",
...
def _report(
    *,
    ...
    report = {
        ...
        "created_by": created_by,
        "dry_run_status": dry_run_status,
        "dry_run_mode": "execution_simulation_only",
        "training_admitted": False,
        "phase1_status": "not_run",
        "dataset_confirmation_changed": False,
        ...
        "dry_run_record_ids": [record["dry_run_record_id"] for record in dry_run_records],
        "dry_run_records": dry_run_records,
...
def _evidence_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Custom Corpus Property Training Admission Execution Dry-Run Evidence",
            "",
            f"- Dry-run status: `{report['dry_run_status']}`",
            f"- Dry-run id: `{report['dry_run_id']}`",
            f"- Execution request id: `{report['execution_request_id']}`",
            f"- Execution request preflight status: `{report['execution_request_preflight_status']}`",
            f"- Execution request status: `{report['execution_request_status']}`",
            f"- Draft precheck status: `{report['draft_precheck_status']}`",
            f"- Draft status: `{report['draft_status']}`",
            f"- Request plan status: `{report['request_plan_status']}`",
            f"- Request preflight status: `{report['request_preflight_status']}`",
            f"- Readiness status: `{report['readiness_status']}`",
            f"- Quarantine run id: `{report['quarantine_run_id']}`",
            f"- Corpus id: `{report['corpus_id']}`",
            f"- Source dry-run id: `{report['source_dry_run_id']}`",
            f"- Admission request id: `{report['admission_request_id']}`",
            f"- Materialization plan id: `{report['materialization_plan_id']}`",
            f"- Source execution request id: `{report['source_execution_request_id']}`",
            f"- Planned candidate count: `{report['planned_candidate_count']}`",
            f"- Execution record count: `{report['execution_record_count']}`",
            f"- Dry-run record count: `{report['dry_run_record_count']}`",
            f"- Dry-run record ids: `{json.dumps(report['dry_run_record_ids'])}`",
            f"- Planned candidate ids: `{json.dumps(report['planned_training_admission_candidate_record_ids'])}`",
            f"- Dry-run errors: `{json.dumps(report['dry_run_errors'])}`",
            f"- Warnings: `{json.dumps(report['warnings'])}`",
            "",
            "## Boundary Statement",
            "",
            "- this is a training admission execution dry-run only.",
...
def _read_safe_json_dict(path: str | Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise CustomCorpusPropertyTrainingAdmissionExecutionDryRunError(f"{label} invalid") from exc
    if not isinstance(payload, dict):
        raise CustomCorpusPropertyTrainingAdmissionExecutionDryRunError(f"{label} invalid")
    if _input_contains_forbidden_material(payload):
        raise CustomCorpusPropertyTrainingAdmissionExecutionDryRunError(f"{label} contains forbidden material")
    return payload
