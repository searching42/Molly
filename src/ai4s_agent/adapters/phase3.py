...
def _parser_markdown_bytes(parsed: ParsedDocument) -> bytes:
...
def _write_markdown(path: Path, parsed: ParsedDocument) -> Path:
    path.write_bytes(_parser_markdown_bytes(parsed))
    return path
...
def _write_parser_outputs(
    *,
    ...
    parsed_json = output_dir / f"{run_id}_parsed_document.json"
    parsed_markdown = output_dir / f"{run_id}_parsed_document.md"
    audit_json = output_dir / f"{run_id}_parser_audit.json"
    audit = {
        "run_id": run_id,
        "adapter": adapter,
    ...
    with _pinned_parser_output_directory(output_dir) as output_descriptor:
        _write_parser_bytes_at(
            output_descriptor,
            parsed_json.name,
            _parser_json_bytes(parsed_doc.model_dump(mode="json")),
        )
        _write_parser_bytes_at(
            output_descriptor,
            parsed_markdown.name,
            _parser_markdown_bytes(parsed_doc),
        )
        _write_parser_bytes_at(
            output_descriptor,
            audit_json.name,
            _parser_json_bytes(audit),
        )
    return {
        "status": "success",
        "adapter": adapter,
...
def _parsed_doc_metadata(
    ...
    *,
    ...
    metadata = {
        ...
        "source_hash": source_hash or _sha256_file(input_pdf),
...
_PINNED_PDF_AFTER_PRECHECK_HOOK: Any = None
_PINNED_OUTPUT_AFTER_PRECHECK_HOOK: Any = None


def _no_follow_flag() -> int:
    value = getattr(os, "O_NOFOLLOW", None)
    if value is None:
        raise ValueError("local PDF parsing requires O_NOFOLLOW support")
    return value


def _open_directory_chain_without_symlinks(directory: Path) -> int:
    ...
    try:
        ...
        raise ValueError("PDF parent path is unavailable or symbolic") from exc
    finally:
        if descriptor != -1:
            os.close(descriptor)


def _open_or_create_directory_chain_without_symlinks(directory: Path) -> int:
    directory = directory.absolute()
    descriptor = os.open(
        directory.anchor,
        os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | _no_follow_flag(),
    ...
    try:
        ...
        raise ValueError("parser output path is unavailable or symbolic") from exc
    finally:
        if descriptor != -1:
            os.close(descriptor)


def _validate_pinned_output_directory(path: Path, descriptor: int) -> None:
    current_descriptor = _open_directory_chain_without_symlinks(path)
    try:
        ...
        if (
            ...
            raise ValueError("parser output directory changed while pinned")
    finally:
        os.close(current_descriptor)


@contextmanager
def _pinned_parser_output_directory(path: Path) -> Iterator[int]:
    absolute = path.absolute()
    descriptor = _open_or_create_directory_chain_without_symlinks(absolute)
    try:
        hook = _PINNED_OUTPUT_AFTER_PRECHECK_HOOK
        if callable(hook):
            hook(absolute)
        try:
            yield descriptor
        finally:
            _validate_pinned_output_directory(absolute, descriptor)
    finally:
        os.close(descriptor)


def _read_parser_bytes_at(directory_descriptor: int, filename: str) -> bytes:
    descriptor = os.open(
        filename,
    ...
    try:
        ...
        if (
            ...
            raise ValueError("parser output is not a stable regular file")
...
def _write_parser_bytes_at(
    directory_descriptor: int,
    filename: str,
    payload: bytes,
) -> None:
    if not filename or Path(filename).name != filename:
        raise ValueError("parser output filename is invalid")
    ...
    try:
        ...
        try:
            ...
            if _read_parser_bytes_at(directory_descriptor, filename) != payload:
                raise ValueError("parser output already exists with different bytes") from None
...
def _parser_json_bytes(payload: dict[str, Any]) -> bytes:
...
@contextmanager
def _pinned_pdf_input(
    path: Path,
    *,
    expected_sha256: str,
) -> Iterator[tuple[BinaryIO, str, os.stat_result]]:
    absolute = path.absolute()
    parent_descriptor = _open_directory_chain_without_symlinks(absolute.parent)
    descriptor = -1
    ...
    try:
        ...
        if (
            ...
            raise ValueError("input PDF is not a stable regular file")
        digest, size = _hash_open_descriptor(descriptor)
        approved = expected_sha256.removeprefix("sha256:").lower()
        if approved and digest != approved:
            raise ValueError("input PDF no longer matches the approved digest")
        hook = _PINNED_PDF_AFTER_PRECHECK_HOOK
        if callable(hook):
            hook(absolute)
        parser_stream = os.fdopen(os.dup(descriptor), "rb")
        yield parser_stream, f"sha256:{digest}", initial
        final_digest, final_size = _hash_open_descriptor(descriptor)
        ...
        if (
            ...
            raise ValueError("input PDF changed while it was being parsed")
    except OSError as exc:
        raise ValueError("input PDF is unavailable, symbolic, or changed") from exc
    finally:
        if parser_stream is not None and not parser_stream.closed:
            parser_stream.close()
        if descriptor != -1:
            os.close(descriptor)
...
def _content_bound_output(
    ...
    *,
    ...
    if not absolute.is_relative_to(root) or absolute == root:
        raise ValueError("parser output escapes the run publication root")
    parent_descriptor = _open_directory_chain_without_symlinks(absolute.parent)
    descriptor = -1
    try:
        descriptor = os.open(
        ...
        if (
            ...
            raise ValueError("parser output changed while it was being bound")
        return {
        ...
        raise ValueError("parser output is unavailable or symbolic") from exc
    finally:
        if descriptor != -1:
            os.close(descriptor)
        os.close(parent_descriptor)
...
def parse_document_pdfplumber_adapter(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_source_sha256 = (
        str(payload.get("expected_source_pdf_sha256") or "").strip().lower()
    )
    ...
    try:
        with _pinned_pdf_input(
            input_pdf,
            expected_sha256=expected_source_sha256,
        ) as (input_stream, source_hash, _):
        ...
        return {
            ...
            "adapter": "parse_document_pdfplumber",
            "error": {
                "code": "source_pdf_integrity_failed",
                "message": str(exc),
            },
        }
...
def parse_pdf_corpus_pdfplumber_adapter(payload: dict[str, Any]) -> dict[str, Any]:
    run_id = str(payload.get("run_id") or "").strip()
    input_dir_raw = str(payload.get("input_pdf_dir") or "").strip()
    output_dir_raw = str(payload.get("output_dir") or "").strip()
    if not run_id or not input_dir_raw or not output_dir_raw:
        return {
            "status": "failed",
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {
    ...
    try:
        input_directory_descriptor = _open_directory_chain_without_symlinks(input_dir)
    except ValueError:
        return {
            "status": "failed",
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {
                "code": "pdf_corpus_not_found",
                "message": "input_pdf_dir must be a non-symlink directory",
            },
        }
    try:
        names = sorted(os.listdir(input_directory_descriptor))
        unsafe = any(
            not stat.S_ISREG(
                os.stat(name, dir_fd=input_directory_descriptor, follow_symlinks=False).st_mode
            )
    ...
    if unsafe:
        return {
            ...
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {
                "code": "unsafe_pdf_corpus_roster",
                "message": "PDF corpus contains an unexpected entry",
            },
        }
    pdfs = [input_dir / name for name in names]
    if not pdfs:
        return {
            "status": "failed",
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {"code": "empty_pdf_corpus", "message": "PDF corpus is empty"},
        }
    if len(pdfs) > 20:
        return {
            "status": "failed",
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {"code": "pdf_corpus_too_large", "message": "PDF corpus exceeds 20 files"},
        }
    expected_corpus = payload.get("expected_corpus")
    if not isinstance(expected_corpus, list) or len(expected_corpus) != len(pdfs):
        return {
            "status": "failed",
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {
                "code": "missing_approved_corpus_roster",
                "message": "exact approved PDF corpus roster is required",
            },
        }
    actual_corpus: list[dict[str, Any]] = []
    try:
        for pdf in pdfs:
            with _pinned_pdf_input(pdf, expected_sha256="") as (_, digest, metadata):
                actual_corpus.append(
                    {
                        "name": pdf.name,
        ...
        return {
            ...
            "adapter": "parse_pdf_corpus_pdfplumber",
            "error": {
                "code": "approved_corpus_roster_changed",
                "message": str(exc),
            },
        }
    if actual_corpus != expected_corpus:
        return {
            ...
            "error": {
                "code": "approved_corpus_roster_changed",
                "message": "PDF corpus no longer matches the approved roster",
            },
        }
    output_dir = _resolve_path(output_dir_raw, base=WORKSPACE)
    with _pinned_parser_output_directory(output_dir):
        pass
    publication_root = output_dir.parent
    documents: list[dict[str, Any]] = []
    for index, (pdf, approved_member) in enumerate(
        zip(pdfs, expected_corpus, strict=True),
        start=1,
    ):
        child_id = f"{run_id}_{index:03d}"
        result = parse_document_pdfplumber_adapter(
            {
                "run_id": child_id,
                "input_pdf": str(pdf),
                "output_dir": str(output_dir / f"document_{index:03d}"),
                "expected_source_pdf_sha256": str(approved_member["sha256"]),
            }
        )
        if result.get("status") != "success":
            return {
                ...
                "adapter": "parse_pdf_corpus_pdfplumber",
                "error": {
                    "code": "corpus_member_parse_failed",
                    "message": f"failed to parse corpus member {index}",
                    "member_index": index,
        ...
        try:
            bound_outputs = {
                "parsed_document": _content_bound_output(
                    Path(str(outputs.get("parsed_document_json") or "")),
                    publication_root=publication_root,
                    artifact_id=f"parsed_document_{index:03d}",
                    kind="parsed_document_json",
                ),
                "parsed_document_markdown": _content_bound_output(
                    Path(str(outputs.get("parsed_document_markdown") or "")),
                    publication_root=publication_root,
                    artifact_id=f"parsed_document_markdown_{index:03d}",
                    kind="parsed_document_markdown",
                ),
                "parser_audit": _content_bound_output(
                    Path(str(outputs.get("parser_audit_json") or "")),
                    publication_root=publication_root,
                    artifact_id=f"parser_audit_{index:03d}",
                    kind="parser_audit_json",
                ),
            }
        except ValueError as exc:
            return {
                "status": "failed",
                "adapter": "parse_pdf_corpus_pdfplumber",
                "error": {"code": "unsafe_parser_output", "message": str(exc)},
            }
        documents.append(
            {
                "member_index": index,
                "source": dict(approved_member),
                "outputs": bound_outputs,
            }
        )
    try:
        for pdf, approved_member in zip(pdfs, expected_corpus, strict=True):
            with _pinned_pdf_input(
                pdf,
                expected_sha256=str(approved_member["sha256"]),
            ):
                pass
    except ValueError as exc:
        return {
    ...
    corpus_audit_payload = {
        "schema_version": "literature_parse_corpus_audit.v1",
        "run_id": run_id,
        "parser_profile": "pdfplumber_local",
        "approved_sources": expected_corpus,
        "document_outputs": documents,
        "created_at": now_iso(),
    }
    corpus_audit_json = output_dir / f"{run_id}_parser_audit.json"
    corpus_audit_bytes = _parser_json_bytes(corpus_audit_payload)
    corpus_audit = {
        "artifact_id": "parser_audit",
        "kind": "corpus_parser_audit_json",
        "relative_path": corpus_audit_json.absolute()
        .relative_to(publication_root.absolute())
        .as_posix(),
        "size_bytes": len(corpus_audit_bytes),
        "sha256": hashlib.sha256(corpus_audit_bytes).hexdigest(),
    }
    manifest = {
    ...
    manifest_json = output_dir / f"{run_id}_parsed_corpus_manifest.json"
    with _pinned_parser_output_directory(output_dir) as output_descriptor:
        _write_parser_bytes_at(
            output_descriptor,
            corpus_audit_json.name,
            corpus_audit_bytes,
        )
        _write_parser_bytes_at(
            output_descriptor,
            manifest_json.name,
            _parser_json_bytes(manifest),
        )
    return {
        "status": "success",
        "adapter": "parse_pdf_corpus_pdfplumber",
        "outputs": {
            "parsed_corpus_manifest_json": str(manifest_json),
            "parser_audit_json": str(corpus_audit_json),
        },
    }
...
def parse_document_mineru_adapter(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    remote_host = str(payload.get("remote_host") or "molly-gpu-main").strip()
    ...
    if not execute and not mineru_output_raw:
        return {
            ...
            "command": command,
            "remote": remote,
            "note": "Set execute=true to run MinerU on molly-gpu-main, or pass mineru_output_dir to normalize existing output.",
        }
