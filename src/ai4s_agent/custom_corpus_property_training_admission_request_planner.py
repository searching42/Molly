...
_SCHEMA_VERSION = "custom_corpus_property_training_admission_request_plan.v1"
...
class CustomCorpusPropertyTrainingAdmissionRequestPlannerError(ValueError):
    pass


def plan_property_training_admission_request(
    *,
    ...
    minimum_planned_candidates: int = 1,
) -> dict[str, Any]:
    ...
    errors = _consistency_errors(
        ...
        minimum_planned_candidates=max(int(minimum_planned_candidates), 0),
        warnings=warnings,
    )
    status = "blocked" if errors else "partial" if warnings else "planned"
    summary = _summary(
    ...
    *,
    ...
    try:
        summary = plan_property_training_admission_request(
            ...
            minimum_planned_candidates=args.minimum_planned_candidates,
        )
    except Exception as exc:
        err.write(f"property training admission request plan invalid: {_safe_exception_message(exc)}\n")
        return 1
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m ai4s_agent.custom_corpus_property_training_admission_request_planner",
        description="Plan a future training admission request for property quarantine candidates.",
    )
    ...
    parser.add_argument("--minimum-planned-candidates", type=int, default=1)
    return parser
...
def _consistency_errors(
    *,
    ...
    minimum_planned_candidates: int,
    warnings: list[str],
) -> list[str]:
    errors: list[str] = []
    _append_errors(errors, _schema_errors(formal, binding, plan_payload, plan, plan_preflight, offline_planner, property_planner, materialization_dry_run, execution_request, execution_summary, execution_preflight, quarantine_candidate, quarantine_summary, quarantine_preflight, readiness))
    _append_errors(errors, _status_errors(source_dry_run, execution_request, execution_preflight, quarantine_candidate, quarantine_summary, quarantine_preflight, readiness, require_ready_readiness, minimum_planned_candidates, warnings))
    _append_errors(errors, _hash_errors(quarantine_candidate, quarantine_summary, execution_preflight, quarantine_preflight, readiness, hashes))
    _append_errors(errors, _id_errors(manifest, source_dry_run, review, admission, formal, binding, plan_payload, plan_preflight, offline_planner, property_planner, materialization_dry_run, execution_request, execution_summary, execution_preflight, quarantine_candidate, quarantine_summary, quarantine_preflight, readiness))
    _append_errors(errors, _record_errors(plan_payload, execution_request, execution_summary, execution_preflight, quarantine_candidate, quarantine_summary, quarantine_preflight, readiness, admission, binding, minimum_planned_candidates))
...
def _status_errors(
    ...
    for status in (
        str(quarantine_candidate.get("materializer_status", "")),
        str(quarantine_summary.get("materializer_status", "")),
        str(readiness.get("quarantine_materializer_status", "")),
    ):
        if status == "failed":
    ...
    if quarantine_preflight.get("preflight_status") == "failed":
    ...
    planned_count = len(_safe_list(readiness.get("planned_training_admission_candidate_record_ids")))
    if planned_count < max(minimum_planned_candidates, 1):
        errors.append("minimum_planned_candidate_count_not_met")
    if readiness_status == "ready" and planned_count == 0:
        errors.append("no_planned_candidates")
    if readiness_status == "ready" and _safe_list(readiness.get("blocked_from_training_admission_record_ids")):
        errors.append("ready_readiness_has_blocked_candidates")

    if (
        readiness.get("training_admitted") is not False
    ...
    if (
        readiness.get("phase1_status") != "not_run"
    ...
    if (
        readiness.get("dataset_confirmation_changed") is not False
...
def _hash_errors(
    ...
    pairs = (
        ...
        ("quarantine_candidate_preflight_summary_sha256", "quarantine_candidate_preflight_summary_sha256", "quarantine_candidate_preflight_summary_sha256_mismatch"),
    )
    candidate_pairs = (
    ...
    for field, hash_field, error in candidate_pairs:
        if quarantine_candidate.get(field) and quarantine_candidate.get(field) != hashes[hash_field]:
            errors.append(error)
    for container in (quarantine_summary, execution_preflight, quarantine_preflight, readiness):
...
def _id_errors(
    ...
    id_sets = (
        ...
        (
            ...
            readiness.get("source_dry_run_id"),
        ...
        (
            ...
            readiness.get("review_manifest_id"),
        ...
        (
            ...
            readiness.get("admission_request_id"),
        ...
        (
            ...
            readiness.get("materialization_plan_id"),
        ...
        (
            ...
            readiness.get("execution_request_id"),
        ...
        (
            readiness.get("quarantine_run_id"),
        ...
        (
            ...
            readiness.get("property_candidate_manifest_id"),
...
def _record_errors(
    ...
    readiness_records = _raw_readiness_records(readiness)
    ...
    planned_ids = _safe_list(readiness.get("planned_training_admission_candidate_record_ids"))
    ...
    if not planned_ids:
        errors.append("no_planned_candidates")
    if len(planned_ids) < max(minimum_planned_candidates, 1):
        errors.append("minimum_planned_candidate_count_not_met")
    if not set(planned_ids).issubset(set(candidate_ids)):
        errors.append("planned_candidate_ids_unknown")

    for container in (quarantine_candidate, quarantine_summary, quarantine_preflight, readiness):
    ...
    for container in (quarantine_summary, quarantine_preflight, readiness):
    ...
    readiness_by_candidate = {str(record.get("candidate_record_id", "")): record for record in readiness_records}
    planned_records = [readiness_by_candidate.get(candidate_id, {}) for candidate_id in planned_ids]
    if any(not record for record in planned_records):
        errors.append("planned_candidate_summary_missing")
    _append_errors(errors, _planned_candidate_source_errors(planned_records, admission, binding))
    return _stable_unique(errors)
...
def _summary(
    *,
    ...
    return {
        ...
        "training_admission_readiness_status": str(readiness.get("readiness_status", "")),
        "quarantine_candidate_preflight_status": str(quarantine_preflight.get("preflight_status", readiness.get("quarantine_candidate_preflight_status", ""))),
        ...
        "planned_request_record_summaries": _planned_request_record_summaries(readiness, planned_ids, hashes["training_admission_readiness_summary_sha256"]),
...
def _planned_request_record_summaries(
    readiness: dict[str, Any],
    planned_ids: list[str],
    readiness_sha256: str,
) -> list[dict[str, Any]]:
    readiness_by_candidate = {str(record.get("candidate_record_id", "")): record for record in _raw_readiness_records(readiness)}
    summaries: list[dict[str, Any]] = []
    for candidate_id in planned_ids:
        record = readiness_by_candidate.get(candidate_id, {})
        if not record:
            continue
        summaries.append(
            {
                ...
                "planned_action": "plan_training_admission_request_candidate",
                "planned_reason": "readiness_ready_for_future_training_admission_request",
                ...
                "training_admission_readiness_sha256": readiness_sha256,
            }
        )
    return summaries
...
def _evidence_markdown(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Custom Corpus Property Training Admission Request Plan Evidence",
            "",
            f"- Planner status: `{summary['planner_status']}`",
            f"- Training admission readiness status: `{summary['training_admission_readiness_status']}`",
            ...
            f"- Planned future training admission candidate ids: `{json.dumps(summary['planned_training_admission_candidate_record_ids'])}`",
            ...
            "## Boundary Statement",
            "",
            "- this is training admission request planning only.",
            "- No training admission request was generated.",
...
def _planned_candidate_source_errors(
...
def _load_source_dry_run_report(path: str | Path) -> CustomCorpusDryRunReport:
    try:
        ...
        raise CustomCorpusPropertyTrainingAdmissionRequestPlannerError("dry-run report invalid") from exc
...
def _read_safe_json_dict(path: str | Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise CustomCorpusPropertyTrainingAdmissionRequestPlannerError(f"{label} invalid") from exc
    if not isinstance(payload, dict):
        raise CustomCorpusPropertyTrainingAdmissionRequestPlannerError(f"{label} invalid")
...
def _raw_readiness_records(readiness: dict[str, Any]) -> list[dict[str, Any]]:
    records = readiness.get("readiness_record_summaries", [])
    return [record for record in records if isinstance(record, dict)] if isinstance(records, list) else []
