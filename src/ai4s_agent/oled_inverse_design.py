...
_REMOTE_V2_PROFILE_ID = "molly-gpu-main-gpu_worker_main-reinvent4-v2"
_REMOTE_COMPUTE_WORKER_MAIN_PROFILE_ID = "molly-compute-main-compute_worker_main-reinvent4-v1"
_REMOTE_PROFILE_ID = _REMOTE_V2_PROFILE_ID
_REMOTE_EXECUTION_POLICIES = {
    _REMOTE_V2_PROFILE_ID: {
        "profile_id": _REMOTE_V2_PROFILE_ID,
        "connection_id": "gpu-worker-main",
        "environment_id": "reinvent4-gpu-main",
        "host_key_policy": "strict_pinned_known_hosts",
        "device_policy": "cpu_only",
        "process_policy": "nice_19_single_thread",
    },
    _REMOTE_COMPUTE_WORKER_MAIN_PROFILE_ID: {
        "profile_id": _REMOTE_COMPUTE_WORKER_MAIN_PROFILE_ID,
        "connection_id": "compute-worker-main",
        "environment_id": "reinvent4-compute-main",
        "host_key_policy": "strict_pinned_known_hosts",
        "device_policy": "cpu_only",
        "process_policy": "nice_19_single_thread",
    },
...
_EXECUTABLE_REMOTE_PROFILE_IDS = frozenset(
    {_REMOTE_V2_PROFILE_ID, _REMOTE_COMPUTE_WORKER_MAIN_PROFILE_ID}
)
_REMOTE_REINVENT_REQUIRED_PLACEHOLDERS = {
    "{{molly_output_csv}}",
...
@dataclass(frozen=True)
class _ResolvedRemoteTransport:
...
def run_oled_inverse_design_from_files(
    *,
    ...
    remote_resolution: _ResolvedRemoteTransport | None = None
    if clean_mode == "remote":
        remote_resolution = _build_remote_transport_contract(
    ...
    with tempfile.TemporaryDirectory(
        ...
        frozen = _materialize_inverse_design_inputs(
            ...
            remote_known_hosts=(
                remote_resolution.known_hosts_path
        ...
        if remote_resolution is not None and (
            frozen.remote_known_hosts_sha256
            != _required_string(
                remote_resolution.public_contract, "known_hosts_sha256"
            )
        ):
        ...
        with _pinned_output_parents_without_symlink_components(root) as pinned:
            design_request_id = _design_request_id(
                ...
                remote_transport_contract_sha256=(
                    _required_string(
                        remote_resolution.public_contract, "contract_sha256"
                    )
                    if remote_resolution is not None
                    else None
                ),
            ...
            transport = _execute_reinvent4_generation(
                ...
                remote_transport_contract=(
                    remote_resolution.public_contract
                    if remote_resolution is not None
                    else None
                ),
                remote_transport_runtime=remote_resolution,
...
def _execute_reinvent4_generation(
    *,
    ...
    remote_transport_runtime: _ResolvedRemoteTransport | None,
    ...
    with tempfile.TemporaryDirectory(
        ...
        if mode == "existing_output":
            ...
            if (
                remote_transport_contract is None
                or remote_transport_runtime is None
                or remote_known_hosts_path is None
            ):
            ...
            backend_payload.update(
                {
                    ...
                    "reinvent4_remote_host_key_alias": remote_transport_runtime.ssh_target,
                    "reinvent4_remote_expected_hostname": (
                        remote_transport_runtime.expected_hostname
                    ),
                    "remote_host": remote_transport_runtime.ssh_target,
                    "remote_repo": remote_transport_runtime.repository_root,
                    "remote_python": remote_transport_runtime.python_path,
                }
            )
...
def _render_remote_reinvent4_config(
    *,
    ...
    try:
        ...
        raise ValueError("rendered REINVENT4 config is not valid TOML") from exc
    if parsed.get("run_type") != "sampling":
        raise ValueError("remote REINVENT4 config must use run_type = sampling")
    if parsed.get("device") != "cpu":
        raise ValueError("remote REINVENT4 config must use device = cpu")
    if parsed.get("seed") != seed:
        raise ValueError("remote REINVENT4 config seed binding is invalid")
    parameters = parsed.get("parameters")
    if not isinstance(parameters, dict) or parameters.get("output_file") != remote_output_csv:
        raise ValueError("remote REINVENT4 output-file binding is invalid")
    json_out_config = parsed.get("json_out_config")
    if (
        not isinstance(json_out_config, str)
        or design_request_id not in json_out_config
        or request_sha256 not in json_out_config
    ):
        raise ValueError(
            "REINVENT4 json_out_config must actively bind the design request ID and SHA-256"
        )
    if PurePosixPath(json_out_config).parent != PurePosixPath(remote_output_csv).parent:
        raise ValueError("REINVENT4 json_out_config must remain inside the remote attempt")
...
def _validate_effective_config_replay(
    *,
    ...
    if profile_id in _EXECUTABLE_REMOTE_PROFILE_IDS:
        renderer = _render_remote_reinvent4_config
    else:
        compatibility = LegacyTransportCompatibilityStore().get_profile(profile_id)
        renderer = (
            _render_legacy_remote_reinvent4_v1_config
            if compatibility.config_renderer == "reinvent4_v1"
            else _render_remote_reinvent4_config
        )
    expected = renderer(
...
def _validated_remote_transport_provenance(
    ...
    *,
    ...
    profile_id = remote.get("profile_id")
    profile = _REMOTE_EXECUTION_POLICIES.get(profile_id)
    if profile is not None:
        ...
        static = {key: remote.get(key) for key in static_keys}
        expected_static = {
            **profile,
            "connection_profile_digest": static["connection_profile_digest"],
            "environment_profile_digest": static["environment_profile_digest"],
            "known_hosts_sha256": static["known_hosts_sha256"],
        }
        digests = (
            static["connection_profile_digest"],
            static["environment_profile_digest"],
            static["known_hosts_sha256"],
        )
    else:
        compatibility = LegacyTransportCompatibilityStore().get_profile(
            str(profile_id or "")
        )
        historical = compatibility.historical_contract()
        static = {
            key: remote.get(key)
            for key in (*sorted(historical), "known_hosts_sha256")
        }
        expected_static = {
            **historical,
            "known_hosts_sha256": static["known_hosts_sha256"],
        }
        digests = (static["known_hosts_sha256"],)
    if (
        any(
...
def _build_remote_transport_contract(
    *,
    ...
    remote_known_hosts: str | Path | None,
) -> _ResolvedRemoteTransport:
    """Resolve private deployment data and freeze only its public binding."""

    profile_id = str(remote_profile_id or _REMOTE_PROFILE_ID).strip()
    if profile_id not in _EXECUTABLE_REMOTE_PROFILE_IDS:
        raise ValueError("inverse-design remote profile is not allowed")
    policy = _REMOTE_EXECUTION_POLICIES[profile_id]
    workspace = Path(os.environ.get("MOLLY_WORKSPACE") or os.getcwd()).expanduser()
    connection = ResourceProfileStore(workspace_dir=workspace).get_connection(
        _required_string(policy, "connection_id")
    )
    environment = RuntimeEnvironmentStore().get_environment(
        _required_string(policy, "environment_id")
    )
    if not connection.enabled:
        raise ValueError("inverse-design connection profile is disabled")
    if environment.connection_id != connection.connection_id:
        raise ValueError("inverse-design environment connection binding is invalid")
    if not connection.known_hosts_path:
        raise ValueError("remote connection profile requires pinned known-hosts")
    configured_known_hosts = Path(
        os.path.abspath(Path(connection.known_hosts_path).expanduser())
    )
    ...
    if remote_known_hosts is not None:
        supplied_bytes, supplied_sha256 = _read_regular_file_bound(
            _absolute_local_path(remote_known_hosts),
            max_bytes=_MAX_INPUT_BYTES,
            reject_symlink_components=True,
        )
        if not supplied_bytes or supplied_sha256 != known_hosts_sha256:
            raise ValueError(
                "remote known-hosts must match the private connection profile"
            )
    public_contract = {
        **policy,
    ...
    public_contract["contract_sha256"] = _sha256_bytes(
        _json_bytes(public_contract)
    )
    return _ResolvedRemoteTransport(
        public_contract=public_contract,
        ssh_target=connection.ssh_host_alias,
