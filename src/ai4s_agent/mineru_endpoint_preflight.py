...
def run_mineru_endpoint_preflight(
    *,
    ...
    if environment.diagnostics["VLLM_USE_FLASHINFER_SAMPLER"]["status"] != "ok":
        warnings.append("gpu_worker_main_hint_vllm_use_flashinfer_sampler")
    if environment.diagnostics["LD_LIBRARY_PATH"]["status"] != "ok":
        warnings.append("gpu_worker_main_hint_ld_library_path_cuda_cudnn_ordering")
...
def collect_environment_diagnostics() -> MinerUEndpointEnvironmentDiagnostics:
    ...
    diagnostics = {
        "VLLM_USE_FLASHINFER_SAMPLER": {
            ...
            "reason": "gpu_worker_main RTX 5090 live MinerU startup required VLLM_USE_FLASHINFER_SAMPLER=0",
        },
        "LD_LIBRARY_PATH": {
            "entries": ld_entries,
