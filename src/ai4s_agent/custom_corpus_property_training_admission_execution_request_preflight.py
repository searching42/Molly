...
_SCHEMA_VERSION = "custom_corpus_property_training_admission_execution_request_preflight.v1"
...
class CustomCorpusPropertyTrainingAdmissionExecutionRequestPreflightError(ValueError):
    pass


def preflight_property_training_admission_execution_request_package(
    *,
    ...
    require_execution_request_written: bool = True,
    ...
    errors = _consistency_errors(
        ...
        hashes=hashes,
        require_execution_request_written=require_execution_request_written,
    ...
    *,
    ...
    if stderr is None:
        ...
        with redirect_stderr(stderr):
            args = parser.parse_args(argv)
    try:
        summary = preflight_property_training_admission_execution_request_package(
            ...
            require_execution_request_written=args.require_execution_request_written,
            allow_execution_request_needs_review=args.allow_execution_request_needs_review,
            minimum_execution_records=args.minimum_execution_records,
        )
    except Exception as exc:
        err.write(f"property training admission execution request preflight invalid: {_safe_exception_message(exc)}\n")
        return 1
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m ai4s_agent.custom_corpus_property_training_admission_execution_request_preflight",
        description="Preflight a property training admission execution request package.",
    )
    ...
    parser.add_argument("--require-execution-request-written", action="store_true", default=True)
    parser.add_argument("--allow-execution-request-needs-review", action="store_true")
...
def _consistency_errors(
    *,
    ...
    hashes: dict[str, str],
    require_execution_request_written: bool,
    allow_execution_request_needs_review: bool,
    ...
    _append_errors(
        ...
        _status_errors(
            ...
            quarantine_preflight,
            quarantine_candidate,
            require_execution_request_written,
            allow_execution_request_needs_review,
            warnings,
        ),
    )
    _append_errors(
...
def _status_errors(
    ...
    _append_errors(errors, _draft_precheck_status_errors(draft_precheck, allow_execution_request_needs_review, warnings))
    _append_errors(errors, _draft_status_errors(draft, "training_admission_request_draft", allow_execution_request_needs_review, warnings))
    _append_errors(
        errors,
        _draft_status_errors(
            draft_summary,
    ...
    _append_errors(
        errors,
        _planned_status_errors(
            request_plan,
            "planner_status",
            "training_admission_request_plan",
            "planned",
            allow_execution_request_needs_review,
            warnings,
        ),
    ...
    if quarantine_status in {"failed", "blocked"}:
        ...
        if allow_execution_request_needs_review:
...
def _request_status_errors(
    ...
    status = str(container.get("request_status", ""))
    if status == "written":
        return []
...
def _draft_precheck_status_errors(container: dict[str, Any], allow: bool, warnings: list[str]) -> list[str]:
    status = str(container.get("precheck_status", ""))
    if status == "passed":
        return []
    if status == "needs_review":
        if allow:
            ...
            return []
        return ["training_admission_request_draft_precheck_needs_review"]
    if status == "blocked":
        return ["training_admission_request_draft_precheck_blocked"]
    return ["training_admission_request_draft_precheck_status_invalid"]
...
def _hash_errors(
    ...
    direct_pairs = (
        ...
        (
            request,
            "source_training_admission_request_draft_summary_sha256",
            "training_admission_request_draft_summary_sha256",
        ),
        (
            request_summary,
            "training_admission_request_draft_summary_sha256",
        ...
        (
            request,
            "source_training_admission_request_draft_precheck_sha256",
        ...
        (
            request,
            "source_quarantine_candidate_preflight_sha256",
            "quarantine_candidate_preflight_summary_sha256",
        ),
        (
            request_summary,
            "quarantine_candidate_preflight_summary_sha256",
...
def _record_errors(
    ...
    top_execution_ids = _safe_list(request.get("execution_record_ids"))
    summary_execution_ids = _safe_list(request_summary.get("execution_record_ids"))
    ...
    draft_planned_ids = _safe_list(draft.get("planned_training_admission_candidate_record_ids"))
    ...
    if len(execution_records) < max(minimum_execution_records, 1):
    ...
    if int(request_summary.get("execution_record_count", len(execution_records))) != len(execution_records):
        errors.append("execution_record_count_mismatch")
    if top_execution_ids != execution_record_ids:
        errors.append("execution_record_ids_mismatch")
    if summary_execution_ids != execution_record_ids:
        errors.append("execution_record_ids_mismatch")
    ...
    if readiness_planned and readiness_planned != request_planned_ids:
        errors.append("planned_candidate_ids_mismatch")
    if draft_planned_ids and draft_planned_ids != request_planned_ids:
        errors.append("planned_candidate_ids_mismatch")
    if not set(request_planned_ids).issubset(set(quarantine_candidate_ids)):
        errors.append("planned_candidate_ids_unknown")
    if int(request_summary.get("planned_candidate_count", len(request_planned_ids))) != len(request_planned_ids):
    ...
    for record in execution_records:
        ...
        if not execution_record_id or not _is_safe_id(execution_record_id):
            errors.append("execution_record_id_invalid")
        if draft_record_id not in draft_record_ids:
            errors.append("execution_record_draft_record_id_unknown")
        ...
        if not _record_values_are_safe(record, allowed_labels={"requested_action", "execution_request_status"}):
            errors.append("execution_record_contains_unsafe_value")
    return _stable_unique(errors)
...
def _summary(
    *,
    ...
    return {
        ...
        "execution_request_id": str(request.get("execution_request_id", request_summary.get("execution_request_id", ""))),
        "corpus_id": str(request.get("corpus_id", request_plan.get("corpus_id", ""))),
        "source_dry_run_id": str(request.get("source_dry_run_id", request_plan.get("source_dry_run_id", ""))),
        "review_manifest_id": str(request.get("review_manifest_id", request_plan.get("review_manifest_id", ""))),
        "admission_request_id": str(request.get("admission_request_id", request_plan.get("admission_request_id", ""))),
        "materialization_plan_id": str(
            request.get("materialization_plan_id", request_plan.get("materialization_plan_id", ""))
        ),
        "source_execution_request_id": str(
            request.get("source_execution_request_id", request_plan.get("execution_request_id", ""))
        ),
        "quarantine_run_id": str(request.get("quarantine_run_id", request_plan.get("quarantine_run_id", ""))),
        "review_queue_id": str(request.get("review_queue_id", request_plan.get("review_queue_id", ""))),
        "property_candidate_manifest_id": str(
            request.get("property_candidate_manifest_id", request_plan.get("property_candidate_manifest_id", ""))
        ),
        "dataset_target": str(request.get("dataset_target", request_plan.get("dataset_target", ""))),
        ...
        "training_admitted": request.get("training_admitted"),
        "phase1_status": str(request.get("phase1_status", "")),
        "dataset_confirmation_changed": request.get("dataset_confirmation_changed"),
        ...
        "blocked_from_training_admission_record_ids": _safe_list(
            request_plan.get("blocked_from_training_admission_record_ids")
        ),
...
def _evidence_markdown(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Custom Corpus Property Training Admission Execution Request Preflight Evidence",
            "",
            ...
            "## Boundary Statement",
            "",
            "- this is a training admission execution request preflight only.",
...
def _read_safe_json_dict(path: str | Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise CustomCorpusPropertyTrainingAdmissionExecutionRequestPreflightError(f"{label} invalid") from exc
    if not isinstance(payload, dict):
        raise CustomCorpusPropertyTrainingAdmissionExecutionRequestPreflightError(f"{label} invalid")
    if _input_contains_forbidden_material(payload):
        raise CustomCorpusPropertyTrainingAdmissionExecutionRequestPreflightError(
...
def _minimal_redaction_failure() -> dict[str, Any]:
    return {
        ...
        "preflight_errors": ["property_training_admission_execution_request_preflight_redaction_failed"],
