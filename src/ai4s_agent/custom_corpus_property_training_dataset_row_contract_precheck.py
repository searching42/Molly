...
_SCHEMA_VERSION = "custom_corpus_property_training_dataset_row_contract_precheck.v1"
_CONTRACT_SCHEMA_VERSION = "custom_corpus_property_training_dataset_row_contract.v1"
_CONTRACT_SUMMARY_SCHEMA_VERSION = "custom_corpus_property_training_dataset_row_contract_builder.v1"
...
class PropertyTrainingDatasetRowContractPrecheckError(ValueError):
    pass


def precheck_property_training_dataset_row_contract(
    *,
    ...
    require_row_contract_written: bool = True,
    allow_row_contract_needs_review: bool = False,
    ...
    _append_errors(
        ...
        _status_errors(
            payloads,
            require_row_contract_written=require_row_contract_written,
            allow_row_contract_needs_review=allow_row_contract_needs_review,
            warnings=warnings,
        ),
    )
    ...
    _append_errors(errors, _record_errors(payloads, minimum_contract_records))
    ...
    *,
    ...
    try:
        summary = precheck_property_training_dataset_row_contract(
            ...
            require_row_contract_written=args.require_row_contract_written,
            allow_row_contract_needs_review=args.allow_row_contract_needs_review,
            minimum_contract_records=args.minimum_contract_records,
        )
    except Exception as exc:
        err.write(f"property training dataset row contract precheck invalid: {_safe_exception_message(exc)}\n")
        return 1
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m ai4s_agent.custom_corpus_property_training_dataset_row_contract_precheck",
        description="Precheck a property training dataset row contract package without writing dataset artifacts.",
    )
    ...
    parser.add_argument("--require-row-contract-written", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--allow-row-contract-needs-review", action="store_true")
...
def _schema_errors(payloads: dict[str, Any]) -> list[str]:
    expected = {
        "training_dataset_row_contract": ("training_dataset_row_contract_schema_invalid", _CONTRACT_SCHEMA_VERSION),
        "training_dataset_row_contract_summary": (
            "training_dataset_row_contract_summary_schema_invalid",
            _CONTRACT_SUMMARY_SCHEMA_VERSION,
        ),
    ...
    return [
        error
        for key, (error, expected_schema) in expected.items()
        if payloads[key].get("schema_version") != expected_schema
    ]
...
def _status_errors(
    ...
    *,
    require_row_contract_written: bool,
    allow_row_contract_needs_review: bool,
    warnings: list[str],
) -> list[str]:
    errors: list[str] = []
    ...
    _append_errors(
        ...
        _status_field_errors(
            ...
            "written" if require_row_contract_written else str(contract.get("contract_status", "")),
            allow_row_contract_needs_review,
            warnings,
        ),
    )
    if contract_summary.get("contract_status") != contract.get("contract_status"):
        errors.append("training_dataset_row_contract_summary_status_mismatch")
    if contract_summary.get("contract_errors"):
        errors.append("training_dataset_row_contract_summary_has_errors")
    _append_errors(
        errors,
        _status_field_errors(
            payloads["training_dataset_materialization_plan_precheck"],
            "precheck_status",
            "training_dataset_materialization_plan_precheck",
            "passed",
            allow_row_contract_needs_review,
    ...
    _append_errors(
        errors,
        _status_field_errors(
            payloads["training_dataset_materialization_plan"],
            "plan_status",
    ...
    _append_errors(
        errors,
        _status_field_errors(
            payloads["training_admission_execution_ledger"],
            "execution_status",
    ...
    for key, field, prefix, required in checks:
        _append_errors(
            errors,
            _status_field_errors(
                payloads[key],
    ...
    if contract.get("contract_mode") != "training_dataset_row_contract_only":
        errors.append("training_dataset_row_contract_mode_invalid")
    if contract.get("contract_errors"):
        errors.append("training_dataset_row_contract_has_errors")
    ...
    for key in ("training_dataset_row_contract", "training_dataset_row_contract_summary"):
        if payloads[key].get("training_admitted") is not True:
            errors.append("training_not_admitted")
    for key in _payload_keys(payloads):
        value = payloads[key]
        if value.get("phase1_status") != "not_run":
            errors.append("phase1_ran")
        if value.get("dataset_confirmation_changed") is not False:
            errors.append("dataset_confirmation_changed")
    post_contract = ("training_dataset_row_contract", "training_dataset_row_contract_summary")
    for key in post_contract:
...
def _contract_structure_errors(payloads: dict[str, Any]) -> list[str]:
    contract = payloads["training_dataset_row_contract"]
    summary = payloads["training_dataset_row_contract_summary"]
    errors: list[str] = []
    required = set(_safe_list(contract.get("required_row_fields")))
    optional = set(_safe_list(contract.get("optional_row_fields")))
    if not _REQUIRED_ROW_FIELDS.issubset(required):
        errors.append("required_row_field_missing")
    if not _OPTIONAL_ROW_FIELDS.issubset(optional):
        errors.append("optional_row_field_missing")
    if _safe_list(summary.get("required_row_fields")) != _safe_list(contract.get("required_row_fields")):
        errors.append("required_row_fields_summary_mismatch")
    if _safe_list(summary.get("optional_row_fields")) != _safe_list(contract.get("optional_row_fields")):
        errors.append("optional_row_fields_summary_mismatch")
    ...
    provenance = contract.get("provenance_field_contract")
    if not isinstance(provenance, dict):
        errors.append("provenance_contract_missing")
    else:
        required_ids = set(_safe_list(provenance.get("required_id_fields")))
        required_shas = set(_safe_list(provenance.get("required_sha_fields")))
        if not {"ledger_record_id", "planned_dataset_record_id", "review_id", "admission_record_id"}.issubset(required_ids):
            errors.append("provenance_contract_id_missing")
        if not {"source_artifact_sha256", "materialization_plan_sha256", "row_contract_sha256"}.issubset(required_shas):
            errors.append("provenance_contract_sha_missing")
    ...
    if not isinstance(quality, dict) or not set(_safe_list(quality.get("allowed_quality_flags"))).issubset(_ALLOWED_QUALITY_FLAGS):
        errors.append("quality_flag_contract_unknown_flag")
    ...
    if not isinstance(split_dedup, dict) or not {"dedup_key", "split_group_key"}.issubset(set(_safe_list(split_dedup.get("required_keys")))):
        errors.append("split_dedup_contract_missing")
    ...
    if not isinstance(model_contract, dict) or not _MODEL_FAMILY_LABELS.issubset(set(model_contract)):
        errors.append("model_family_compatibility_missing")
    ...
    if (
        ...
        or _contains_forbidden_material(output_contract)
    ):
        errors.append("output_format_compatibility_invalid")
    if not contract.get("dataset_name") or not _is_safe_id(str(contract.get("dataset_name", ""))):
        errors.append("dataset_name_invalid")
    if not contract.get("contract_version_label") or not _is_safe_id(str(contract.get("contract_version_label", ""))):
...
def _hash_errors(payloads: dict[str, Any], hashes: dict[str, str]) -> list[str]:
    ...
    source_hash_keys = [
        ...
        if key not in {"training_dataset_row_contract_path", "training_dataset_row_contract_summary_path"}
    ]
    for hash_key in source_hash_keys:
        for container in (contract, contract_summary):
...
def _id_errors(payloads: dict[str, Any]) -> list[str]:
    ...
    groups = {
        ...
        "row_contract_id": (
            ("training_dataset_row_contract", "training_dataset_row_contract_summary"),
            ("row_contract_id",),
        ),
        "execution_ledger_id": (
            (
        ...
        "dry_run_id": (_payload_keys(payloads), ("dry_run_id",)),
        "review_manifest_id": (_payload_keys(payloads), ("review_manifest_id",)),
        "admission_request_id": (_payload_keys(payloads), ("admission_request_id",)),
        "review_queue_id": (_payload_keys(payloads), ("review_queue_id",)),
        "property_candidate_manifest_id": (_payload_keys(payloads), ("property_candidate_manifest_id",)),
        "dataset_name": (
            (
                "training_dataset_row_contract",
                "training_dataset_row_contract_summary",
    ...
    for logical, (payload_keys, field_keys) in groups.items():
        values = _group_values(payloads, payload_keys, field_keys)
        if len(set(values)) > 1:
...
def _group_values(
    payloads: dict[str, Any],
    payload_keys: tuple[str, ...] | list[str],
    field_keys: tuple[str, ...],
) -> list[str]:
    values: list[str] = []
    for payload_key in payload_keys:
        for field_key in field_keys:
            value = str(payloads[payload_key].get(field_key, ""))
            if value:
                values.append(value)
                break
    return values
...
def _record_errors(payloads: dict[str, Any], minimum_contract_records: int) -> list[str]:
    errors: list[str] = []
    contract = payloads["training_dataset_row_contract"]
    contract_summary = payloads["training_dataset_row_contract_summary"]
    ...
    planned_dataset_ids = [str(record.get("planned_dataset_record_id", "")) for record in plan_records]
    ...
    reference_ledger_ids = [str(record.get("ledger_record_id", "")) for record in references]
    planned_candidate_ids = _safe_list(plan.get("planned_training_admission_candidate_record_ids"))
    reference_candidate_ids = [str(record.get("candidate_record_id", "")) for record in references]
    if not references or len(references) < max(minimum_contract_records, 1):
    ...
    if int(contract_summary.get("contract_record_reference_count", -1)) != len(references):
        errors.append("contract_record_reference_count_mismatch")
    if int(plan.get("planned_dataset_record_count", -1)) != len(plan_records):
        errors.append("planned_dataset_record_count_mismatch")
    if len(references) != len(plan_records):
        errors.append("contract_record_reference_count_mismatch")
    if reference_dataset_ids != planned_dataset_ids:
        errors.append("planned_dataset_record_ids_mismatch")
    if set(reference_ledger_ids) != set(ledger_record_ids):
        errors.append("ledger_record_ids_mismatch")
    if set(reference_candidate_ids) != set(planned_candidate_ids):
        errors.append("planned_candidate_ids_mismatch")
    for ids in (
        _safe_list(contract.get("planned_dataset_record_ids")),
        _safe_list(contract_summary.get("planned_dataset_record_ids")),
        _safe_list(plan.get("planned_dataset_record_ids")),
    ):
        if ids and ids != planned_dataset_ids:
            errors.append("planned_dataset_record_ids_mismatch")
    for ids in (
        _safe_list(contract.get("planned_training_admission_candidate_record_ids")),
        _safe_list(contract_summary.get("planned_training_admission_candidate_record_ids")),
        ...
        _safe_list(readiness.get("planned_training_admission_candidate_record_ids")),
    ):
        if ids and ids != planned_candidate_ids:
            errors.append("planned_candidate_ids_mismatch")
    if not set(planned_candidate_ids).issubset(set(_safe_list(quarantine_candidate.get("candidate_record_ids")))):
...
def _summary(
    ...
    result: dict[str, Any] = {
        ...
        **_source_fields(paths, hashes),
        ...
        "materialization_plan_id": str(plan.get("materialization_plan_id", contract.get("materialization_plan_id", ""))),
        "execution_ledger_id": str(ledger.get("execution_ledger_id", contract.get("execution_ledger_id", ""))),
        "dry_run_id": str(ledger.get("dry_run_id", contract.get("dry_run_id", ""))),
        "execution_request_id": str(ledger.get("execution_request_id", contract.get("execution_request_id", ""))),
        "corpus_id": str(ledger.get("corpus_id", contract.get("corpus_id", ""))),
        "source_dry_run_id": str(ledger.get("source_dry_run_id", contract.get("source_dry_run_id", ""))),
        "review_manifest_id": str(ledger.get("review_manifest_id", contract.get("review_manifest_id", ""))),
        "admission_request_id": str(ledger.get("admission_request_id", contract.get("admission_request_id", ""))),
        "review_queue_id": str(ledger.get("review_queue_id", contract.get("review_queue_id", ""))),
        "property_candidate_manifest_id": str(
            ledger.get("property_candidate_manifest_id", contract.get("property_candidate_manifest_id", ""))
        ),
        ...
        "training_admitted": contract.get("training_admitted") is True,
        "training_dataset_materialized": contract.get("training_dataset_materialized") is True,
        "dataset_artifact_created": contract.get("dataset_artifact_created") is True,
        "phase1_status": str(contract.get("phase1_status", "")),
        "dataset_confirmation_changed": contract.get("dataset_confirmation_changed") is True,
        "required_row_fields": _safe_list(contract.get("required_row_fields")),
        "optional_row_fields": _safe_list(contract.get("optional_row_fields")),
        "contract_record_reference_count": len(_safe_records(contract.get("contract_record_references"))),
        "planned_dataset_record_count": int(summary.get("planned_dataset_record_count", 0) or 0),
        "planned_dataset_record_ids": _safe_list(summary.get("planned_dataset_record_ids")),
...
def _markdown(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Custom Corpus Property Training Dataset Row Contract Precheck Evidence",
            "",
...
def _read_safe_json_dict(path: str | Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise PropertyTrainingDatasetRowContractPrecheckError(f"{label} invalid") from exc
    if not isinstance(payload, dict):
        raise PropertyTrainingDatasetRowContractPrecheckError(f"{label} invalid")
    if _input_contains_forbidden_material(payload):
        raise PropertyTrainingDatasetRowContractPrecheckError(f"{label} contains forbidden material")
    return payload
