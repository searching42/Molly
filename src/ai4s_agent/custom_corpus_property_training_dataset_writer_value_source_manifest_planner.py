...
_MANIFEST_SCHEMA_VERSION = "custom_corpus_property_training_dataset_writer_value_source_manifest.v1"
_SUMMARY_SCHEMA_VERSION = "custom_corpus_property_training_dataset_writer_value_source_manifest_planner.v1"
...
class PropertyTrainingDatasetWriterValueSourceManifestPlannerError(ValueError):
    pass


def plan_property_training_dataset_writer_value_source_manifest(
    *,
    ...
    output_dir: str | Path,
    value_source_manifest_id: str,
    created_by: str,
    confirm_training_dataset_writer_value_source_manifest: bool = False,
    allow_input_binding_preflight_needs_review: bool = False,
    minimum_value_source_records: int = 1,
    require_all_bound_value_fields_covered: bool = True,
) -> dict[str, Any]:
    ...
    run_dir = Path(output_dir).expanduser() / value_source_manifest_id
    warnings: list[str] = []
    value_source_records = _value_source_records(payloads, paths)
    errors = _validation_errors(
        ...
        confirm=confirm_training_dataset_writer_value_source_manifest,
        allow_needs_review=allow_input_binding_preflight_needs_review,
        minimum_value_source_records=minimum_value_source_records,
        require_all_bound_value_fields_covered=require_all_bound_value_fields_covered,
        value_source_records=value_source_records,
        warnings=warnings,
    )
    ...
    if _contains_forbidden_material({"manifest": manifest, "summary": summary, "markdown": markdown}):
    ...
    *,
    ...
    try:
        summary = plan_property_training_dataset_writer_value_source_manifest(
            ...
            value_source_manifest_id=args.value_source_manifest_id,
            created_by=args.created_by,
            confirm_training_dataset_writer_value_source_manifest=args.confirm_training_dataset_writer_value_source_manifest,
            allow_input_binding_preflight_needs_review=args.allow_input_binding_preflight_needs_review,
            minimum_value_source_records=args.minimum_value_source_records,
            require_all_bound_value_fields_covered=args.require_all_bound_value_fields_covered,
        )
    except Exception as exc:
        err.write(f"property training dataset writer value source manifest planner invalid: {_safe_exception_message(exc)}\n")
        return 1
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m ai4s_agent.custom_corpus_property_training_dataset_writer_value_source_manifest_planner",
        description="Plan a property training dataset writer value source manifest without reading source payloads.",
    )
    ...
    parser.add_argument("--value-source-manifest-id", required=True)
    parser.add_argument("--created-by", required=True)
    parser.add_argument("--confirm-training-dataset-writer-value-source-manifest", action="store_true")
    parser.add_argument("--allow-input-binding-preflight-needs-review", action="store_true")
    parser.add_argument("--minimum-value-source-records", type=int, default=1)
    parser.add_argument("--require-all-bound-value-fields-covered", action=argparse.BooleanOptionalAction, default=True)
    return parser
...
def _validation_errors(
    ...
    *,
    ...
    require_all_bound_value_fields_covered: bool,
    ...
    if not _is_safe_id(value_source_manifest_id):
        errors.append("value_source_manifest_id_invalid")
    ...
    record_errors = _record_errors(payloads, minimum_value_source_records, require_all_bound_value_fields_covered)
    if allow_needs_review:
        for warning_code in ("no_value_source_records", "missing_value_source_for_bound_required_field"):
    ...
    _append_errors(errors, _value_source_record_errors(value_source_records, payloads, hashes))
...
def _status_errors(payloads: dict[str, Any], allow_needs_review: bool, warnings: list[str]) -> list[str]:
    ...
    errors.extend(
        _status_field_errors(
            payloads["training_dataset_writer_input_binding_plan_preflight"],
            "preflight_status",
            "training_dataset_writer_input_binding_plan_preflight",
            "passed",
            allow_needs_review,
    ...
    if payloads["training_dataset_writer_input_binding_plan_preflight"].get("preflight_errors"):
        errors.append("training_dataset_writer_input_binding_plan_preflight_has_errors")
    errors.extend(
        _status_field_errors(
    ...
    if plan.get("planner_errors") or payloads["training_dataset_writer_input_binding_planner_summary"].get("planner_errors"):
...
def _record_errors(
    ...
    errors: list[str] = []
    preflight = payloads["training_dataset_writer_input_binding_plan_preflight"]
    plan = payloads["training_dataset_writer_input_binding_plan"]
    ...
    if int(planner.get("binding_record_count", -1)) != len(binding_records):
        errors.append("binding_record_count_mismatch")
    if int(preflight.get("binding_record_count", -1)) != len(binding_records):
        errors.append("binding_record_count_mismatch")
    ...
    if planner.get("binding_record_ids") and _safe_list(planner.get("binding_record_ids")) != binding_ids:
        errors.append("binding_record_ids_mismatch")
    if preflight.get("binding_record_ids") and _safe_list(preflight.get("binding_record_ids")) != binding_ids:
        errors.append("binding_record_ids_mismatch")
    writer_ids = [str(record.get("writer_request_record_id", "")) for record in binding_records]
    if _safe_list(plan.get("writer_request_record_ids")) != writer_ids:
        errors.append("writer_request_record_ids_mismatch")
    if _safe_list(preflight.get("writer_request_record_ids")) != writer_ids:
        errors.append("writer_request_record_ids_mismatch")
    ...
    if _safe_list(preflight.get("row_preview_ids")) != row_preview_ids:
        errors.append("row_preview_ids_mismatch")
    planned_candidates = _safe_list(plan.get("planned_training_admission_candidate_record_ids"))
    if planned_candidates != [str(record.get("candidate_record_id", "")) for record in binding_records]:
    ...
    value_source_count = 0
    missing_value_fields = 0
    for record in binding_records:
        record_value_bindings = []
        for binding in _safe_records(record.get("required_field_bindings")):
            field_name = str(binding.get("field_name", ""))
            if field_name in _VALUE_FIELD_NAMES:
                if binding.get("binding_status") == "bound":
                    record_value_bindings.append(binding)
                    value_source_count += 1
                else:
                    missing_value_fields += 1
        present_bound_fields = {str(binding.get("field_name", "")) for binding in record_value_bindings}
        if require_all_bound_value_fields_covered and _VALUE_FIELD_NAMES - present_bound_fields:
            missing_value_fields += len(_VALUE_FIELD_NAMES - present_bound_fields)
        _append_errors(errors, _binding_record_errors(record))
    if value_source_count < max(minimum_value_source_records, 1):
...
def _binding_record_errors(record: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if _value_contains_unsafe_material(record):
        errors.append("binding_record_contains_unsafe_value")
    if record.get("binding_record_status") not in {"planned", "needs_review"}:
        errors.append("binding_record_status_invalid")
    for field in ("writer_executed", "training_dataset_materialized", "dataset_artifact_created", "dataset_confirmation_changed"):
    ...
    if record.get("phase1_status") != "not_run":
        errors.append("phase1_ran")
    for binding_group in ("required_field_bindings", "optional_field_bindings"):
        for binding in _safe_records(record.get(binding_group)):
            _append_errors(errors, _field_binding_errors(binding))
    if isinstance(record.get("requested_output_formats"), list):
        if not set(_safe_list(record.get("requested_output_formats"))).issubset(_OUTPUT_FORMAT_LABELS):
            errors.append("output_format_label_invalid")
    return errors


def _field_binding_errors(binding: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    field_name = str(binding.get("field_name", ""))
    status = str(binding.get("binding_status", ""))
    if field_name and not _is_safe_id(field_name):
        errors.append("field_name_invalid")
    if status not in _FIELD_BINDING_STATUSES:
        errors.append("field_binding_status_invalid")
    if status == "bound":
        ...
        if binding.get("source_artifact_sha256") and not _SHA_RE.match(str(binding.get("source_artifact_sha256"))):
            errors.append("source_artifact_sha256_invalid")
        if not source_record_id or not _is_safe_id(source_record_id):
...
def _value_source_record_errors(
    ...
    for record in records:
        ...
        if expected_path_key:
            ...
            if record.get("source_artifact_basename") != Path(payloads["paths"][expected_path_key]).name:
                errors.append("source_artifact_basename_mismatch")
        for field in ("source_authorized_for_future_writer",):
...
def _value_source_records(payloads: dict[str, Any], paths: dict[str, str | Path]) -> list[dict[str, Any]]:
    plan = payloads["training_dataset_writer_input_binding_plan"]
    records: list[dict[str, Any]] = []
    for binding_record in _safe_records(plan.get("binding_records")):
        ...
        for binding in _safe_records(binding_record.get("required_field_bindings")):
            field_name = str(binding.get("field_name", ""))
            if field_name not in _VALUE_FIELD_NAMES or binding.get("binding_status") != "bound":
                continue
            label = str(binding.get("source_artifact_label", ""))
            path_key = _SOURCE_LABEL_PATH_KEYS.get(label, "")
            basename = Path(paths[path_key]).name if path_key else ""
            records.append(
                {
                    "value_source_record_id": f"{binding_record_id}-value-source-{field_name}",
                    "writer_input_binding_record_id": binding_record_id,
                    "writer_request_record_id": str(binding_record.get("writer_request_record_id", "")),
                    "row_preview_id": str(binding_record.get("row_preview_id", "")),
                    "candidate_record_id": str(binding_record.get("candidate_record_id", "")),
                    "record_id": str(binding_record.get("record_id", "")),
                    "document_id": str(binding_record.get("document_id", "")),
                    "field_name": str(binding_record.get("field_name", "")),
                    "value_field_name": field_name,
                    "source_artifact_label": label,
                    "source_artifact_basename": basename,
                    "source_artifact_sha256": str(binding.get("source_artifact_sha256", "")),
                    "source_record_id": str(binding.get("source_record_id", "")),
                    "derivation_rule": str(binding.get("derivation_rule", "")),
                    "source_authorized_for_future_writer": True,
...
def _manifest(
    *,
    ...
    plan = payloads["training_dataset_writer_input_binding_plan"]
    record_ids = [str(record.get("value_source_record_id", "")) for record in value_source_records]
    return {
        ...
        "manifest_mode": "training_dataset_writer_value_source_manifest_only",
        ...
        **_source_status_fields(payloads),
        ...
        "training_admitted": True if status in {"planned", "needs_review"} else plan.get("training_admitted") is True,
        "training_dataset_materialized": False,
        "dataset_artifact_created": False,
        "phase1_status": "not_run",
        "dataset_confirmation_changed": False,
        "value_source_record_count": len(value_source_records),
        "value_source_record_ids": record_ids,
        ...
        "value_source_records": value_source_records,
        "value_field_coverage_summary": _value_field_coverage_summary(value_source_records),
        "missing_value_source_field_counts": _missing_value_source_field_counts(plan),
        "planner_errors": _stable_unique(errors),
        "warnings": _stable_unique(warnings),
        "redaction_status": "passed",
        "boundary_statement": [
            "writer value source manifest only",
            "writer not executed",
...
def _summary(
    *,
    ...
    return {
        ...
        "planner_status": status,
        "value_source_manifest_id": value_source_manifest_id,
        "training_dataset_writer_value_source_manifest_path": manifest_path.name,
        "training_dataset_writer_value_source_manifest_sha256": manifest_sha256,
        **_source_fields(paths, hashes),
        **_source_ids(plan),
        **_source_status_fields(payloads),
        "dataset_name": str(plan.get("dataset_name", "")),
        ...
        "dataset_confirmation_changed": False,
        "value_source_record_count": len(value_source_records),
        "value_source_record_ids": [str(record.get("value_source_record_id", "")) for record in value_source_records],
        "binding_record_ids": _safe_list(plan.get("binding_record_ids")),
        "writer_request_record_ids": _safe_list(plan.get("writer_request_record_ids")),
        "planned_training_admission_candidate_record_ids": _safe_list(
            plan.get("planned_training_admission_candidate_record_ids")
...
def _missing_value_source_field_counts(plan: dict[str, Any]) -> dict[str, int]:
    counts = {field: 0 for field in sorted(_VALUE_FIELD_NAMES)}
    for record in _safe_records(plan.get("binding_records")):
        bound = {
            str(binding.get("field_name", ""))
            for binding in _safe_records(record.get("required_field_bindings"))
            if binding.get("binding_status") == "bound"
        }
        for field in _VALUE_FIELD_NAMES - bound:
            counts[field] += 1
    return counts


def _source_ids(plan: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "source_execution_request_id": str(plan.get("source_execution_request_id", "")),
    }
...
def _markdown(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Custom Corpus Property Training Dataset Writer Value Source Manifest Evidence",
            "",
            ...
            "## Boundary Statement",
            "",
            "- this is a training dataset writer value source manifest only.",
...
def _read_safe_json_dict(path: str | Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise PropertyTrainingDatasetWriterValueSourceManifestPlannerError(f"{label} invalid") from exc
    if not isinstance(payload, dict):
        raise PropertyTrainingDatasetWriterValueSourceManifestPlannerError(f"{label} invalid")
    if _input_contains_forbidden_material(payload):
        raise PropertyTrainingDatasetWriterValueSourceManifestPlannerError(f"{label} contains forbidden material")
    return payload
