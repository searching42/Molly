...
from test_custom_corpus_property_training_dataset_materialization_dry_run import (
    _kwargs as _dry_run_kwargs,
)
from test_custom_corpus_property_training_dataset_materialization_dry_run import (
    _write_dry_run_package as _write_base_dry_run_package,
)
...
def test_valid_full_package_returns_passed_and_writes_outputs(tmp_path: Path) -> None:
    ...
    summary_path = tmp_path / "property-training-dataset-materialization-dry-run-precheck-summary.json"
    markdown_path = tmp_path / "property-training-dataset-materialization-dry-run-precheck-summary.md"
    ...
    assert summary["schema_version"] == "custom_corpus_property_training_dataset_materialization_dry_run_precheck.v1"
    assert summary["precheck_status"] == "passed"
    assert written_summary == summary
    assert summary["materialization_dry_run_id"] == "property-training-dataset-dry-run-001"
    ...
    assert set(summary["field_coverage_summary"]["required_row_fields"]) == REQUIRED_ROW_FIELDS
    assert set(summary["field_coverage_summary"]["optional_row_fields"]) == OPTIONAL_ROW_FIELDS
    assert summary["model_family_compatibility_summary"]["conformers_generated"] is False
    assert summary["model_family_compatibility_summary"]["dpa3_structures_generated"] is False
    assert summary["output_format_compatibility_summary"]["jsonl_created"] is False
    assert summary["output_format_compatibility_summary"]["parquet_created"] is False
    assert summary["output_format_compatibility_summary"]["lmdb_created"] is False
    assert summary["output_format_compatibility_summary"]["csv_created"] is False
    ...
    assert "this is a training dataset materialization dry-run precheck only" in markdown
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("training_dataset_materialization_dry_run_summary", lambda payload: payload.__setitem__("schema_version", "wrong"), "training_dataset_materialization_dry_run_summary_schema_invalid"),
...
def test_schema_and_status_failures_block(tmp_path: Path, target: str, mutator: object, error_code: str) -> None:
    ...
    summary = precheck_property_training_dataset_materialization_dry_run(**_kwargs(paths))
...
def test_dry_run_needs_review_blocks_by_default_and_can_be_allowed(tmp_path: Path) -> None:
    ...
    blocked = precheck_property_training_dataset_materialization_dry_run(**_kwargs(paths))
    allowed = precheck_property_training_dataset_materialization_dry_run(
        **_kwargs(paths),
        allow_dry_run_needs_review=True,
    )

    assert blocked["precheck_status"] == "blocked"
    assert "training_dataset_materialization_dry_run_needs_review" in blocked["precheck_errors"]
    assert allowed["precheck_status"] == "needs_review"
    assert "training_dataset_materialization_dry_run_needs_review" in allowed["warnings"]


@pytest.mark.parametrize(
    ("target", "field", "error_code"),
    [
        ("training_dataset_materialization_dry_run_summary", "training_dataset_materialization_dry_run_report_sha256", "training_dataset_materialization_dry_run_report_sha256_mismatch"),
        ("training_dataset_materialization_dry_run_report", "training_dataset_row_contract_precheck_sha256", "training_dataset_row_contract_precheck_sha256_mismatch"),
        ...
        ("training_dataset_materialization_dry_run_report", "training_dataset_materialization_plan_sha256", "training_dataset_materialization_plan_sha256_mismatch"),
        ("training_dataset_materialization_dry_run_report", "training_admission_execution_ledger_sha256", "training_admission_execution_ledger_sha256_mismatch"),
    ],
)
...
def test_dry_run_boundary_failures_block(tmp_path: Path, field: str, value: object, error_code: str) -> None:
    paths = _write_precheck_package(tmp_path)
    _mutate_json(paths["training_dataset_materialization_dry_run_report"], lambda payload: payload.__setitem__(field, value))
...
@pytest.mark.parametrize(
    ("mutator", "error_code"),
    [
        (lambda payload: payload.__setitem__("row_preview_count", 2), "row_preview_count_mismatch"),
        (lambda payload: payload.__setitem__("row_preview_ids", ["other-row-preview"]), "row_preview_ids_mismatch"),
        (lambda payload: payload.__setitem__("row_previews", []), "no_row_previews"),
        (lambda payload: payload["row_previews"][0].__setitem__("contract_record_reference_id", "other-reference"), "contract_record_reference_ids_mismatch"),
        (lambda payload: payload["row_previews"][0].__setitem__("planned_dataset_record_id", "other-dataset-record"), "planned_dataset_record_ids_mismatch"),
        (lambda payload: payload["row_previews"][0].__setitem__("property_value", "raw value 0.72"), "row_preview_contains_unsafe_value"),
        (lambda payload: payload["row_previews"][0].__setitem__("notes", "serialized dataset row"), "row_preview_contains_unsafe_value"),
        (lambda payload: payload["row_previews"][0].__setitem__("output_path", "future.jsonl"), "row_preview_contains_unsafe_value"),
    ],
)
def test_row_preview_integrity_failures_block(tmp_path: Path, mutator: object, error_code: str) -> None:
    paths = _write_precheck_package(tmp_path)
    _mutate_json(paths["training_dataset_materialization_dry_run_report"], mutator)
...
@pytest.mark.parametrize(
    ("mutator", "error_code"),
    [
        (lambda payload: payload.pop("field_coverage_summary"), "field_coverage_summary_missing"),
        (lambda payload: payload["field_coverage_summary"].__setitem__("required_row_fields", ["dataset_record_id"]), "field_coverage_required_fields_mismatch"),
        (lambda payload: payload.pop("model_family_compatibility_summary"), "model_family_compatibility_summary_missing"),
        (lambda payload: payload["model_family_compatibility_summary"].__setitem__("conformers_generated", True), "conformers_generated"),
        (lambda payload: payload["model_family_compatibility_summary"].__setitem__("dpa3_structures_generated", True), "dpa3_structures_generated"),
        (lambda payload: payload.pop("output_format_compatibility_summary"), "output_format_compatibility_summary_missing"),
        (lambda payload: payload["output_format_compatibility_summary"].__setitem__("jsonl_created", True), "jsonl_created"),
        (lambda payload: payload["output_format_compatibility_summary"].__setitem__("parquet_created", True), "parquet_created"),
        (lambda payload: payload["output_format_compatibility_summary"].__setitem__("lmdb_created", True), "lmdb_created"),
        (lambda payload: payload["output_format_compatibility_summary"].__setitem__("csv_created", True), "csv_created"),
    ],
)
def test_summary_sections_are_enforced(tmp_path: Path, mutator: object, error_code: str) -> None:
    paths = _write_precheck_package(tmp_path)
    _mutate_json(paths["training_dataset_materialization_dry_run_report"], mutator)
...
def test_redaction_fail_closed_writes_no_unsafe_markdown(
    ...
    paths = _write_precheck_package(tmp_path)
    summary_path = tmp_path / "precheck-summary.json"
    markdown_path = tmp_path / "precheck-summary.md"
    monkeypatch.setattr(
        "ai4s_agent.custom_corpus_property_training_dataset_materialization_dry_run_precheck._contains_forbidden_material",
        lambda value: True,
    )
    ...
    assert summary == {
        "schema_version": "custom_corpus_property_training_dataset_materialization_dry_run_precheck.v1",
...
def _write_precheck_package(tmp_path: Path, *, needs_review: bool = False) -> dict[str, Path]:
    paths = _write_base_dry_run_package(tmp_path, needs_review=needs_review)
    dry_run_summary = run_property_training_dataset_materialization_dry_run(
        **_dry_run_kwargs(paths),
        confirm_training_dataset_materialization_dry_run=True,
        allow_row_contract_precheck_needs_review=needs_review,
    )
    ...
    paths["training_dataset_materialization_dry_run_report"] = run_dir / "property_training_dataset_materialization_dry_run_report.json"
    paths["training_dataset_materialization_dry_run_summary"] = run_dir / "property_training_dataset_materialization_dry_run_summary.json"
    return paths
