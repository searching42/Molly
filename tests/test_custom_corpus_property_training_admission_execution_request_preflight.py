...
from test_custom_corpus_property_training_admission_execution_request import (
    _kwargs as _execution_request_kwargs,
)
from test_custom_corpus_property_training_admission_execution_request import (
    _write_execution_request_package,
)
...
def test_valid_full_package_returns_passed_and_writes_outputs(tmp_path: Path) -> None:
    ...
    written = json.loads(paths["training_execution_request_preflight_summary"].read_text(encoding="utf-8"))
    markdown = paths["training_execution_request_preflight_markdown"].read_text(encoding="utf-8")
    assert written == summary
    assert summary["schema_version"] == "custom_corpus_property_training_admission_execution_request_preflight.v1"
    ...
    assert summary["execution_record_ids"][0].startswith(
        "property-training-admission-execution-request-001-property-training-admission-request-draft-001-"
    )
    assert summary["execution_record_ids"][0].endswith("property-candidate-001")
    assert summary["preflight_errors"] == []
    assert summary["warnings"] == []
    assert summary["redaction_status"] == "passed"
    assert summary["training_admission_execution_request_path"] == paths["training_execution_request"].name
    assert str(tmp_path) not in json.dumps(summary, sort_keys=True)
    assert "this is a training admission execution request preflight only" in markdown
...
def test_blocking_input_failures(tmp_path: Path, target: str, mutator: object, error_code: str) -> None:
    ...
    summary = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    assert summary["preflight_status"] == "blocked"
    assert error_code in summary["preflight_errors"]


def test_execution_request_needs_review_blocks_by_default_and_can_return_needs_review(tmp_path: Path) -> None:
    ...
    blocked = preflight_property_training_admission_execution_request_package(**_kwargs(paths))
    allowed = preflight_property_training_admission_execution_request_package(
        **_kwargs(paths),
        allow_execution_request_needs_review=True,
    )

    assert blocked["preflight_status"] == "blocked"
    assert "training_admission_execution_request_needs_review" in blocked["preflight_errors"]
    assert allowed["preflight_status"] == "needs_review"
    assert "training_admission_execution_request_needs_review" in allowed["warnings"]


def test_sha_and_id_mismatch_block(tmp_path: Path) -> None:
    paths = _write_preflight_package(tmp_path)
    _mutate_json(
        paths["training_execution_request_summary"],
        lambda payload: payload.__setitem__("training_admission_execution_request_sha256", "sha256:" + "0" * 64),
    )
    sha_summary = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    paths = _write_preflight_package(tmp_path / "id")
    _mutate_json(paths["training_execution_request"], lambda payload: payload.__setitem__("corpus_id", "other-corpus"))
    id_summary = preflight_property_training_admission_execution_request_package(**_kwargs(paths))
...
def test_record_consistency_failures(tmp_path: Path) -> None:
    ...
    no_records = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    paths = _write_preflight_package(tmp_path / "no-planned")
    ...
    no_planned = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    paths = _write_preflight_package(tmp_path / "count")
    _mutate_json(paths["training_execution_request"], lambda payload: payload.__setitem__("execution_record_count", 2))
    count_mismatch = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    paths = _write_preflight_package(tmp_path / "ids")
    _mutate_json(paths["training_execution_request"], lambda payload: payload.__setitem__("execution_record_ids", ["wrong-id"]))
    id_mismatch = preflight_property_training_admission_execution_request_package(**_kwargs(paths))

    assert "no_execution_records" in no_records["preflight_errors"]
    assert "no_planned_candidates" in no_planned["preflight_errors"]
    assert "execution_record_count_mismatch" in count_mismatch["preflight_errors"]
    assert "execution_record_ids_mismatch" in id_mismatch["preflight_errors"]
...
def test_summary_and_markdown_do_not_include_temp_paths_or_artifact_paths(tmp_path: Path) -> None:
    paths = _write_preflight_package(tmp_path)
...
def test_redaction_fail_closed_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    paths = _write_preflight_package(tmp_path)
    monkeypatch.setattr(
        "ai4s_agent.custom_corpus_property_training_admission_execution_request_preflight._contains_forbidden_material",
        lambda value: True,
    )
    ...
    assert summary == {
        "schema_version": "custom_corpus_property_training_admission_execution_request_preflight.v1",
        "preflight_status": "blocked",
        "preflight_errors": ["property_training_admission_execution_request_preflight_redaction_failed"],
        "redaction_status": "failed",
    }
    assert not paths["training_execution_request_preflight_markdown"].exists()
...
def test_cli_stdout_valid_json_and_no_training_artifacts_created(tmp_path: Path) -> None:
    ...
    code = main(
        ...
        + [
            ...
            "--output-markdown",
            str(paths["training_execution_request_preflight_markdown"]),
        ],
        stdout=stdout,
        stderr=stderr,
    )
...
def _write_preflight_package(
    ...
    *,
    ...
    summary = build_property_training_admission_execution_request(
        **_execution_request_kwargs(paths, execution_request_id=execution_request_id),
        confirm_training_admission_execution_request_output=True,
        allow_draft_precheck_needs_review=allow_execution_request_needs_review,
    )
    assert summary["request_status"] in {"written", "needs_review"}
    run_dir = paths["training_execution_request_output_dir"] / execution_request_id
    paths["training_execution_request"] = run_dir / "property_training_admission_execution_request.json"
    paths["training_execution_request_summary"] = run_dir / "property_training_admission_execution_request_summary.json"
    paths["training_execution_request_evidence"] = run_dir / "redacted_property_training_admission_execution_request_evidence.md"
    paths["training_execution_request_preflight_summary"] = tmp_path / "property_training_admission_execution_request_preflight_summary.json"
    paths["training_execution_request_preflight_markdown"] = tmp_path / "property_training_admission_execution_request_preflight_summary.md"
    return paths
