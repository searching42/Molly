...
def test_generate_candidates_reinvent4_backend_executes_remote_config_and_normalizes_output(
    ...
    def fake_run_argv_cmd(*, argv: list[str], cwd: Path, timeout_sec: int = 120) -> dict[str, object]:
        calls.append(argv)
        if argv[0] == "scp" and argv[-2].startswith("molly-gpu-main:"):
    ...
    assert any(
        call[0] == "ssh"
        and call[-2] == "molly-gpu-main"
        and "reinvent.Reinvent" in call[-1]
        for call in calls
    )
    execution_commands = [
        call[-1]
    ...
    result = phase1_module._generate_candidates_reinvent4_backend(
        {
            ...
            "reinvent4_remote_output_csv": f"{attempt_dir}/output.csv",
            "reinvent4_remote_known_hosts_file": str(known_hosts),
            "reinvent4_remote_host_key_alias": "molly-gpu-main",
...
def test_generate_candidates_reinvent4_backend_uses_private_environment_profile(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    ResourceProfileStore(workspace_dir=tmp_path, config_dir=config_dir).save_connection(
        ConnectionProfile(
            ...
            declared_capabilities=["gpu", "reinvent4"],
        )
    )
    ...
    result = generate_candidates_stub_adapter(
        {
            ...
            "actor": "user",
            "execute": True,
            "environment_profile_id": "reinvent4-default",
        }
    )
...
def test_unimol_legacy_remote_training_executes_scp_and_ssh(tmp_path: Path, monkeypatch) -> None:
    ...
    assert calls[0][-1] == "molly-gpu-main:/remote/tmp/r-remote_plqy_train.csv"
    assert calls[2][-2] == "molly-gpu-main"
    ...
    assert report["remote"]["host"] == "molly-gpu-main"
...
def test_unimol_legacy_prediction_passes_remote_runtime_env(tmp_path: Path, monkeypatch) -> None:
    ...
    assert env["UNIMOL_REMOTE_HOST"] == "molly-gpu-main"
