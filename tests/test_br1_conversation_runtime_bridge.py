...
def _br1_plan_response() -> dict[str, Any]:
    ...
    # Exercise a non-default authorized evaluation top_n so the final result
    # projection cannot silently fall back to its intermediate default.
    response["task_options"]["evaluate_private_structured_dataset_canary_v1"][
        "top_n"
    ] = 2
    return response
...
def test_br1_conversation_front_door_drives_synthetic_remote_chain(
    ...
    assert results[0]["task_type"] == "evaluate_private_structured_dataset_canary_v1"
    assert results[0]["output_contract"] == "computational-top-n-v1"
    assert results[0]["summary_statistics"]["top_n"] == 2
    assert results[0]["summary_statistics"]["ranked_candidate_count"] <= 2
    assert all(
        item["task_type"] in {
            "evaluate_private_structured_dataset_canary_v1",
            "generate_private_reinvent4_v1",
    ...
    assert {
        item["task_type"] for item in results[1:]
    } == {
        "generate_private_reinvent4_v1",
        "predict_private_unimol_v1",
    }
    ...
    assert all(
        item["summary_statistics"]["candidate_count"] == 2
