...
from test_custom_corpus_property_quarantine_materializer import (
    _kwargs as _quarantine_kwargs,
)
from test_custom_corpus_property_quarantine_materializer import (
    _write_quarantine_package,
)


def test_valid_full_package_returns_passed_and_writes_optional_outputs(tmp_path: Path) -> None:
    paths = _write_candidate_preflight_package(tmp_path)
    ...
    written_summary = json.loads(paths["candidate_preflight_summary"].read_text(encoding="utf-8"))
    markdown = paths["candidate_preflight_markdown"].read_text(encoding="utf-8")
    assert written_summary == summary
    assert summary["schema_version"] == "custom_corpus_property_quarantine_candidate_preflight.v1"
    ...
    assert "this is quarantine candidate preflight only" in markdown
...
def test_quarantine_needs_review_returns_needs_review_by_default_and_fails_in_strict_mode(tmp_path: Path) -> None:
    paths = _write_candidate_preflight_package(tmp_path, package_binding_status="needs_review", allow_quarantine_needs_review=True)

    loose = preflight_property_quarantine_candidates(**_kwargs(paths))
    strict = preflight_property_quarantine_candidates(**_kwargs(paths), require_no_needs_review=True)

    assert loose["preflight_status"] == "needs_review"
    assert "quarantine_materializer_needs_review" in loose["warnings"]
    assert strict["preflight_status"] == "failed"
    assert "quarantine_materializer_needs_review" in strict["preflight_errors"]
...
def test_schema_mismatch_in_each_input_fails(tmp_path: Path, target: str, mutator: object, error_code: str) -> None:
    ...
    _refresh_hash_references(paths, target)

    summary = preflight_property_quarantine_candidates(**_kwargs(paths))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("quarantine_candidate_records", lambda payload: payload.__setitem__("materialization_mode", "training"), "quarantine_materialization_mode_invalid"),
        ("quarantine_candidate_records", lambda payload: payload.__setitem__("materializer_status", "failed"), "quarantine_materializer_failed"),
        ...
        ("quarantine_materializer_summary", lambda payload: payload.__setitem__("materializer_errors", ["bad"]), "quarantine_materializer_has_errors"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("preflight_status", "failed"), "execution_preflight_failed"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("preflight_errors", ["bad"]), "execution_preflight_has_errors"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("materializer_status", "success"), "materializer_status_not_run"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("phase1_status", "success"), "phase1_ran"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("training_admitted", True), "training_admitted"),
        ("execution_preflight_summary", lambda payload: payload.__setitem__("dataset_confirmation_changed", True), "dataset_confirmation_changed"),
    ],
)
def test_status_and_boundary_failures(tmp_path: Path, target: str, mutator: object, error_code: str) -> None:
    paths = _write_candidate_preflight_package(tmp_path)
    _mutate_json(paths[target], mutator)
    _refresh_hash_references(paths, target)
...
@pytest.mark.parametrize(
    ("field", "target", "error_code"),
    [
        ("source_manifest_sha256", "quarantine_candidate_records", "manifest_sha256_mismatch"),
        ("source_dry_run_report_sha256", "quarantine_candidate_records", "dry_run_report_sha256_mismatch"),
        ("source_review_manifest_sha256", "quarantine_candidate_records", "review_manifest_sha256_mismatch"),
        ("source_admission_request_sha256", "quarantine_candidate_records", "admission_request_sha256_mismatch"),
        ("source_formal_package_validation_sha256", "quarantine_candidate_records", "formal_package_validation_sha256_mismatch"),
        ("source_property_package_binding_summary_sha256", "quarantine_candidate_records", "property_package_binding_summary_sha256_mismatch"),
        ("source_materialization_plan_sha256", "quarantine_candidate_records", "materialization_plan_sha256_mismatch"),
        ("source_materialization_preflight_summary_sha256", "quarantine_candidate_records", "materialization_plan_preflight_summary_sha256_mismatch"),
        ("source_offline_planner_output_sha256", "quarantine_candidate_records", "offline_planner_output_sha256_mismatch"),
        ("source_property_planner_summary_sha256", "quarantine_candidate_records", "property_planner_summary_sha256_mismatch"),
        ("source_materialization_dry_run_report_sha256", "quarantine_candidate_records", "materialization_dry_run_report_sha256_mismatch"),
        ("source_execution_request_sha256", "quarantine_candidate_records", "execution_request_sha256_mismatch"),
        ("source_execution_request_summary_sha256", "quarantine_candidate_records", "execution_request_summary_sha256_mismatch"),
        ("source_execution_preflight_summary_sha256", "quarantine_candidate_records", "execution_preflight_summary_sha256_mismatch"),
        ("quarantine_candidate_records_sha256", "quarantine_materializer_summary", "quarantine_candidate_records_sha256_mismatch"),
    ],
)
def test_hash_mismatches_fail(tmp_path: Path, field: str, target: str, error_code: str) -> None:
    paths = _write_candidate_preflight_package(tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("quarantine_candidate_records", lambda payload: payload.__setitem__("candidate_record_count", 2), "candidate_record_count_mismatch"),
        ("quarantine_candidate_records", lambda payload: payload.__setitem__("candidate_record_ids", ["other-record"]), "candidate_record_ids_mismatch"),
        ("quarantine_materializer_summary", lambda payload: payload.__setitem__("candidate_record_ids", ["other-record"]), "candidate_record_ids_mismatch"),
        ("quarantine_materializer_summary", lambda payload: payload.__setitem__("materialization_record_ids", ["other-record"]), "materialization_record_ids_mismatch"),
        ("quarantine_materializer_summary", lambda payload: payload.__setitem__("execution_record_ids", ["other-record"]), "execution_record_ids_mismatch"),
...
def test_candidate_record_source_failures(tmp_path: Path, record_id: str, error_code: str) -> None:
    paths = _write_candidate_preflight_package(tmp_path, include_needs_review=True)
...
def test_candidate_records_contain_safe_ids_hashes_only_plus_redacted_summaries(tmp_path: Path) -> None:
    paths = _write_candidate_preflight_package(tmp_path)

    summary = preflight_property_quarantine_candidates(**_kwargs(paths))
    candidate = json.loads(paths["quarantine_candidate_records"].read_text(encoding="utf-8"))
    record = candidate["candidate_records"][0]
    serialized = json.dumps({"summary": summary, "candidate": candidate}, sort_keys=True)

    assert record["candidate_status"] == "quarantined"
    assert record["materialization_boundary"] == [
        "candidate_only",
        "not_training",
        "not_phase1",
        "dataset_confirmation_unchanged",
    ]
    assert record["normalized_value_summary"] == "short normalized value summary"
    assert record["provenance_summary"] == "short provenance summary"
...
def test_redaction_fail_closed_writes_no_markdown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    paths = _write_candidate_preflight_package(tmp_path)
    monkeypatch.setattr(
        "ai4s_agent.custom_corpus_property_quarantine_candidate_preflight._contains_forbidden_material",
        lambda value: True,
    )
    stdout = io.StringIO()

    code = main(
        _cli_args(paths) + ["--output-markdown", str(paths["candidate_preflight_markdown"])],
        stdout=stdout,
        stderr=io.StringIO(),
    )
    ...
    assert code == 1
    assert summary == {
        "schema_version": "custom_corpus_property_quarantine_candidate_preflight.v1",
        "preflight_status": "failed",
        "preflight_errors": ["property_quarantine_candidate_preflight_redaction_failed"],
        "redaction_status": "failed",
    }
    assert not paths["candidate_preflight_markdown"].exists()
...
def _write_candidate_preflight_package(
    tmp_path: Path,
    *,
    ...
    paths = _write_quarantine_package(
        tmp_path,
        package_binding_status=package_binding_status,
        include_needs_review=include_needs_review,
    )
    quarantine_summary = run_property_quarantine_materializer(
        **_quarantine_kwargs(paths),
        confirm_quarantine_materialization=True,
        allow_execution_preflight_needs_review=allow_quarantine_needs_review,
    )
    assert quarantine_summary["materializer_status"] in {"written", "needs_review"}
    run_dir = paths["quarantine_output_dir"] / "property-quarantine-materializer-001"
    paths["quarantine_candidate_records"] = run_dir / "property_quarantine_candidate_records.json"
    paths["quarantine_materializer_summary"] = run_dir / "property_quarantine_materializer_summary.json"
    paths["candidate_preflight_summary"] = tmp_path / "property_quarantine_candidate_preflight_summary.json"
    paths["candidate_preflight_markdown"] = tmp_path / "property_quarantine_candidate_preflight_summary.md"
    return paths
...
def _refresh_hash_references(paths: dict[str, Path], target: str) -> None:
