...
def _configure_private_remote_profile(
    *,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    profile_id: str,
    ...
    policy = inverse_runner._REMOTE_EXECUTION_POLICIES[profile_id]  # type: ignore[attr-defined]
    config_dir = tmp_path / "private-config"
    monkeypatch.setenv("MOLLY_CONFIG_DIR", str(config_dir))
    known_hosts = tmp_path / f"known-hosts-{policy['connection_id']}"
    ...
    ResourceProfileStore(
        ...
        ConnectionProfile(
            connection_id=str(policy["connection_id"]),
            ssh_host_alias=ssh_target,
            expected_hostname=expected_hostname,
            remote_root="/remote/runs",
            known_hosts_path=str(known_hosts),
            declared_capabilities=["reinvent4"],
        )
    )
    RuntimeEnvironmentStore(config_dir=config_dir).save_environment(
        RuntimeEnvironmentProfile(
            ...
            connection_id=str(policy["connection_id"]),
            repository_root="/remote/apps/reinvent4",
            python_path="/remote/envs/reinvent4/bin/python",
...
@pytest.mark.parametrize(
    ...
    [
        ("molly-gpu-main-gpu_worker_main-reinvent4-v2", "molly-gpu-main", "gpu-worker-main"),
        ("molly-compute-main-compute_worker_main-reinvent4-v1", "molly-compute-main", "compute-worker-main"),
    ],
)
def test_remote_mode_renders_an_isolated_reinvent4_attempt_without_real_transport(
    ...
    known_hosts = _configure_private_remote_profile(
        tmp_path=tmp_path,
    ...
    def fake_transport(
        ...
        *,
        ...
        assert payload["remote_repo"] == "/remote/apps/reinvent4"
        assert payload["remote_python"] == "/remote/envs/reinvent4/bin/python"
    ...
    persisted_transport = receipt["generator"]["provenance"]["remote_transport"]
    assert "ssh_target" not in persisted_transport
    assert "expected_hostname" not in persisted_transport
    assert "repo" not in persisted_transport
    assert "python" not in persisted_transport
    assert persisted_transport["connection_profile_digest"].startswith("sha256:")
    assert persisted_transport["environment_profile_digest"].startswith("sha256:")
...
def test_legacy_v1_remote_publication_remains_exactly_replayable(
    ...
    config_dir.mkdir()
    monkeypatch.setenv("MOLLY_CONFIG_DIR", str(config_dir))
    legacy_profile_id = "retired-example-reinvent4-v1"
    legacy_ssh_target = "retired-example-alias"
    legacy_hostname = "retired-example-host"
    legacy_repo = "/srv/example-molly/reinvent4"
    legacy_python = "/srv/example-molly/envs/reinvent4/bin/python"
    known_hosts = tmp_path / "known_hosts"
    known_hosts.write_text(
        f"{legacy_ssh_target} ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAITest\n",
        encoding="utf-8",
    )
    (config_dir / "legacy_transport_profiles.json").write_text(
        json.dumps(
            {
                "schema_version": LEGACY_TRANSPORT_PROFILE_SCHEMA_VERSION,
                "profiles": [
                    {
                        "legacy_profile_id": legacy_profile_id,
                        "ssh_target": legacy_ssh_target,
                        "expected_hostname": legacy_hostname,
                        "repository_root": legacy_repo,
    ...
    historical_contract = {
        ...
        "host_key_policy": "strict_pinned_known_hosts",
        "known_hosts_sha256": inverse_runner._sha256_bytes(known_hosts.read_bytes()),  # type: ignore[attr-defined]
    }
    historical_contract["contract_sha256"] = inverse_runner._sha256_bytes(  # type: ignore[attr-defined]
        inverse_runner._json_bytes(  # type: ignore[attr-defined]
            {key: value for key, value in historical_contract.items() if key != "contract_sha256"}
        )
    )

    def historical_resolution(**_: object) -> object:
        return inverse_runner._ResolvedRemoteTransport(  # type: ignore[attr-defined]
            public_contract=historical_contract,
            ssh_target=legacy_ssh_target,
            expected_hostname=legacy_hostname,
            repository_root=legacy_repo,
            python_path=legacy_python,
    ...
    with monkeypatch.context() as legacy:
        legacy.setattr(
            inverse_runner,
            "_build_remote_transport_contract",
        ...
        result = run_oled_inverse_design_from_files(
            ...
            reinvent4_mode="remote",
            seed=29,
            remote_profile_id=legacy_profile_id,
            remote_known_hosts=known_hosts,
            generated_at="2026-07-21T12:05:00+08:00",
        )
    ...
    receipt_bytes = (result.output_dir / "inverse_design.json").read_bytes()
    assert receipt["generator"]["provenance"]["remote_transport"]["profile_id"] == legacy_profile_id
    verified = inverse_runner.verify_oled_inverse_design_publication_from_files(
        inverse_design_json=result.output_dir / "inverse_design.json",
        batch_selection_json=batch_receipt,
        screening_receipt_json=publication.screening_receipt,
    ...
    assert verified.design_request_id == result.design_request_id
    assert verified.publication_id == result.publication_id
    assert (result.output_dir / "inverse_design.json").read_bytes() == receipt_bytes
...
def test_new_remote_run_explicitly_rejects_legacy_v1_profile(
    ...
    known_hosts = _configure_private_remote_profile(
        tmp_path=tmp_path,
        monkeypatch=monkeypatch,
        profile_id=inverse_runner._REMOTE_PROFILE_ID,  # type: ignore[attr-defined]
        ssh_target="molly-gpu-main",
