...
def test_remote_worker_registry_plans_safe_non_executable_assignment(tmp_path: Path) -> None:
    registry = RemoteWorkerRegistry(workspace_dir=tmp_path, config_dir=tmp_path / "config")
    registry.save_worker(
        RemoteWorkerConfig(
            worker_id="molly-gpu-main-mineru",
            transport="ssh",
            host="molly-gpu-main",
    ...
    assignment = registry.plan_assignment(
        RemoteWorkerRequest(
            ...
            preferred_worker_id="molly-gpu-main-mineru",
    ...
    assert assignment.worker_id == "molly-gpu-main-mineru"
    assert assignment.matched_capabilities == ["gpu", "mineru_parse"]
    assert assignment.missing_capabilities == []
    assert assignment.required_permissions == ["remote_worker:molly-gpu-main-mineru", "external_network:ssh"]
...
def test_remote_worker_api_registers_worker_and_returns_assignment(tmp_path: Path) -> None:
    ...
    created = client.post(
        ...
        json={
            ...
            "transport": "ssh",
            "host": "molly-gpu-main",
            "display_name": "GPU worker MinerU",
            "capabilities": ["gpu", "mineru_parse"],
            "environment": "MinerU",
        },
    )
    assert created.status_code == 200
    assert created.json["worker"]["worker_id"] == "molly-gpu-main-mineru"

    listed = client.get("/api/workers")
    assert listed.status_code == 200
    assert listed.json["workers"][0]["worker_id"] == "molly-gpu-main-mineru"
