...
import ai4s_agent.execution_agent as execution_agent_module
...
def test_execution_tool_catalog_projects_pending_task_option_schema(
    ...
    # The serialized catalog (what the LLM actually sees and what the store
    # byte-verifies) must omit the null field entirely, not just on the
    # Python object.  Parent-model serialization uses the Pydantic serializer
    # of each tool, so a missing schema must not surface as ``null``.
    serialized = catalog.model_dump(mode="json")
    pause = next(
        item
        for item in serialized["tools"]
        if item["tool_id"] == "agent.pause_current.v1"
    )
    assert "option_schema" not in pause
    advance = next(
        item
        for item in serialized["tools"]
        if item["tool_id"] == "controller.advance_current.v1"
    )
    assert advance["option_schema"] == option_schema
    ...
    plain_serialized = plain_catalog.model_dump(mode="json")
    assert all("option_schema" not in tool for tool in plain_serialized["tools"])


def test_historical_execution_agent_publication_replays_byte_exact(
    tmp_path,
) -> None:
    """A pre-PR Execution Agent proposal publication must still read through
    the store with exact byte verification (tool catalogs had no
    ``option_schema`` field)."""
    ...
    fixture_root = (
        ...
        / "tests"
        / "fixtures"
        / "historical_execution_agent_publication"
    )
    manifest = json.loads(
    ...
    target = (
        storage.project_dir("project-1")
        / "agent_execution_agent_proposals"
        / manifest["tool_call_proposal_id"]
    )
    ...
    for name in (
        ...
        (target / name).write_bytes((fixture_root / name).read_bytes())

    publication = ExecutionAgentStore(storage=storage).read_proposal(
        project_id="project-1",
        tool_call_proposal_id=manifest["tool_call_proposal_id"],
    )
    assert publication.proposal.selected_tool_id == "agent.pause_current.v1"
    assert publication.proposal.prompt_version == EXECUTION_AGENT_PROMPT_VERSION_V1
    serialized_tools = publication.tool_catalog.model_dump(mode="json")["tools"]
    assert all("option_schema" not in tool for tool in serialized_tools)
    # Regenerated bytes equal the persisted bytes, which is exactly what the
    # store's publication-manifest verification enforces.
    expected = _pretty_json_bytes(
...
def test_v1_prompt_publication_replays_after_current_prompt_moves_to_v2(
    ...
    created = service.create_proposal(
        ...
        request=_proposal_request(
            initial.execution.execution_digest,
            request_id="historical-v1-prompt-publication",
        ),
        provider=_provider("agent.pause_current.v1"),
        provider_binding_digest=_agent_digest({"provider": "stub"}),
    )
    historical = created.publication
    expected_v1_digest = execution_agent_prompt_digest(
        ...
        prompt_version=EXECUTION_AGENT_PROMPT_VERSION_V1,
    )
    assert historical.proposal.prompt_version == EXECUTION_AGENT_PROMPT_VERSION_V1
    assert historical.proposal.prompt_digest == expected_v1_digest
