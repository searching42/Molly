...
- Phase 1 status: project sidebar, project creation/selection, project chat composer, stable per-project chat `run_id`, conversation next-turn decision call, explicit research-source proposal action, explicit modeling-plan action, response console, review artifacts, and collapsed advanced raw API tools are implemented in the Flask web UI. Remaining UI work is visual polish, richer inline artifact rendering, persistent conversation history, and a simple execution/gate review surface.
...
- [ ] Require `ModelingAgent` to consult project memory, previous run diagnostics, built-in domain rules, and optional user-approved web/literature search before proposing preprocessing, target transforms, split strategy, backend, and hyperparameters.
  - MVP status: project memory, previous diagnostics, trainability context, built-in OLED rules, `user_approved_external_search` policy labels, structured `TargetEvidenceItem` injection, ResearchAgent cited-summary handoff, `/api/agent/modeling-plan` target brief output/storage, dialogue-to-payload preparation, `ConversationTurnDecision` next-turn routing, conversation-to-`ResearchSourceProposal` dry-run planning, Flask chat UI next-turn/research-source proposal controls, and `ResearchAcquisitionPreparation` review-only adapter payload preparation are wired. Remaining TODO: connect confirmed acquisition preparation to gated execution/resume without bypassing existing adapter gates.
- [x] Add a conversation next-turn decision layer that uses ordinary chat to ask follow-up questions, collect explicit evidence approval, and decide when a modeling-plan proposal can be generated.
- [x] Add a conversation-to-research-source bridge that turns chat DOI/URL mentions into a reviewable, non-executable `ResearchSourceProposal` before any external acquisition.
- [x] Add `ResearchAcquisitionPreparation` so a reviewed source proposal can produce explicit source-manifest/acquisition adapter payloads, required gates, and required permissions before any adapter execution.
...
- [x] Let `ResearchAgent` provide target-specific evidence only through cited summaries and structured source manifests; do not inject web/literature claims directly into preprocessing or hyperparameters without a reviewable `TargetModelingBrief`.
  - MVP status: `ConversationAgent.prepare_modeling_plan_payload()` extracts DOI/URL mentions and approval state from ordinary dialogue, uses `available_inputs` to recognize dynamic targets such as `homo`/`lumo`/`soc_rate`, and keeps unrelated approvals from approving external evidence; `ConversationAgent.decide_next_turn()` now returns `needs_clarification`, `needs_evidence_approval`, or `ready_for_modeling_plan`; the Flask chat UI uses that next-turn decision as the user-facing boundary before modeling-plan generation; `ConversationAgent.prepare_research_source_payload()` and `/api/agent/conversation/research-sources` can produce a dry-run `ResearchSourceProposal` from an explicit UI action; `ResearchAgent.prepare_acquisition()` and `/api/agent/research-acquisition/prepare` can produce review-only adapter payloads for source manifest/acquisition preparation; `ResearchAgent.prepare_target_evidence_items()` converts approved cited summaries with DOI/URL/source refs into `TargetEvidenceItem` records; `/api/agent/modeling-plan` passes those items into `TargetModelingBrief` artifacts when `user_approved_external_search=True`. Network acquisition remains outside these methods and still requires user approval.
...
- [x] Include referenced input artifact content digests in execution snapshots so training/prediction inputs cannot change after approval.
- [x] Scope gate approval to the paused task snapshot: reject extra future gates, consume approval after the task, and require a new snapshot when later tasks reuse the same gate name.
- [x] Write execute-ready snapshots for plan-only remote tasks so execution resumes through `/api/run-plan/resume` instead of direct adapter calls.
...
- [x] Reject direct execution for gated adapters even when a registered task policy exists; gated execution must go through `RunPlanExecutor` snapshot approval.
